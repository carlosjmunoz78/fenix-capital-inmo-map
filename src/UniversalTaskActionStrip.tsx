...
export default function UniversalTaskActionStrip(){
 ...
 useEffect(()=>{if(!scope){setHost(null);return}let alive=true;fetchAppApi<Ctx>('/session/context').then(r=>alive&&setCtx(r.status===200?r.data:null));const place=()=>{const root=document.querySelector('.ops-root .ops-content') as HTMLElement|null;if(!root)return;let node=root.querySelector(':scope > .task-action-strip-host') as HTMLElement|null;if(!node){node=document.createElement('section');node.className='task-action-strip-host'}const quick=root.querySelector(':scope > .ops-shared-quick,:scope > .ops-uniform-footer-host');if(quick){if(node.parentElement!==root||node.nextElementSibling!==quick)root.insertBefore(node,quick)}else if(node.parentElement!==root||node!==root.lastElementChild)root.appendChild(node);setHost(node);setTitle(root.querySelector('h1')?.textContent?.trim()||'')};place();const obs=new MutationObserver(place);obs.observe(document.body,{childList:true,subtree:true});return()=>{alive=false;obs.disconnect();document.querySelectorAll('.task-action-strip-host').forEach(n=>n.remove());setHost(null)}},[scope?.type,scope?.code,location.pathname]);
