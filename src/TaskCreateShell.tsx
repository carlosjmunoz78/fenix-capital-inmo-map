...
export default function TaskCreateShell(){
 ...
 useEffect(()=>{if(!active||!logged)return;let alive=true;(async()=>{const c=await fetchAppApi<Ctx>('/session/context');if(!alive)return;const context=c.status===200?c.data:null;setCtx(context);if(context?.role!=='Direccion'){setTarget(context?.actor_code||'');setAssignees([]);return;}const[p,v]=await Promise.all([fetchAppApi<PersonalResponse>('/personal'),fetchAppApi<VisitadoresResponse>('/visitadores')]);if(!alive)return;const self=context.actor_code?[{actor_code:context.actor_code,name:'Dirección',role:'Direccion'}]:[];const financials=(p.status===200?p.data?.items??[]:[]).flatMap(x=>x.actor_code?[{actor_code:x.actor_code,name:x.name?.trim()||x.actor_code,role:x.role?.trim()||'Financiero'}]:[]);const visitors=(v.status===200?v.data?.items??[]:[]).flatMap(x=>x.actor_code?[{actor_code:x.actor_code,name:x.nombre?.trim()||x.actor_code,role:x.rol?.trim()||'Visitador'}]:[]);const options=[...self,...financials,...visitors].filter((x,i,a)=>a.findIndex(y=>y.actor_code===x.actor_code)===i).sort((a,b)=>a.name.localeCompare(b.name,'es'));setAssignees(options);if(!target&&context.actor_code)setTarget(context.actor_code);})();return()=>{alive=false};},[active,logged,target]);
 ...
 async function submit(e:FormEvent){e.preventDefault();if(!valid)return;if(!preview){setPreview(true);setMessage('');return;}setBusy(true);setMessage('');const r=await createTask(payload);setBusy(false);if((r.status===201||r.status===200)&&r.data?.ok){setCreated(r.data);setMessage('Tarea creada en la fuente canónica y auditada.');setPreview(false);}else if(r.status===403)setMessage('Tu perfil no puede crear esta tarea con ese responsable.');else setMessage(`No se pudo crear la tarea (${r.data?.error||r.status}).`);}
 ...
 return <OperationalShellFrame
  ...
  topbar={topbar}
 >
   <div className="ops-title"><div><span className="ops-icon"><CalendarDays size={20}/></span><div><h1>Nueva tarea</h1><p>Alta canónica con responsable explícito y confirmación previa.</p></div></div><span className="ops-live ok">{IS_PRODUCTION?'PROD':'PRE-PROD'}</span></div>
