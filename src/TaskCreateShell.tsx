...
export default function TaskCreateShell(){
 ...
 const initialInstruction=useMemo(()=>new URLSearchParams(location.search).get('instruction')?.trim()||'',[location.search]);
 ...
 const[assignees,setAssignees]=useState<Assignee[]>([]),[title,setTitle]=useState(initialInstruction),[target,setTarget]=useState(''),[criticality,setCriticality]=useState(''),[due,setDue]=useState('');
 ...
 useEffect(()=>{if(active&&initialInstruction){setTitle(initialInstruction);setPreview(false);setCreated(null)}},[active,initialInstruction]);
