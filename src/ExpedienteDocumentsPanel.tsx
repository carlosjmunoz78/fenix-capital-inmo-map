...
export default function ExpedienteDocumentsPanel({expedienteId}:{expedienteId:string}){
 ...
 const[loading,setLoading]=useState(true),[status,setStatus]=useState<number|null>(null),[rows,setRows]=useState<Row[]>([]);
 useEffect(()=>{
  let alive=true;
  setLoading(true);
 ...
 return <section className="exp-documents-panel" aria-label="Documentación del expediente" data-testid="expediente-documents-panel">
  <div className="exp-documents-head"><div><span>DOCUMENTACIÓN DEL EXPEDIENTE</span><h2>Archivos ya vinculados a este expediente</h2><p>Solo se muestran documentos cuya relación canónica corresponde exactamente a este expediente.</p></div><strong data-testid="expediente-document-count">{loading?'Cargando…':`${docs.length} documento${docs.length===1?'':'s'}`}</strong></div>
