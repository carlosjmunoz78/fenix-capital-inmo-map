...
export default function ContactDetailShell(){
 ...
 return <OperationalShellFrame className="contact-detail-root" theme={theme} navigation={effectiveNav} activeRoute="/contactos" anaSubtitle="Cuando quieras, avanzamos paso a paso." anaRoute="/ana" query="" onQueryChange={()=>{}} searchPlaceholder="Buscar expediente, cliente o tarea..." searchActionLabel="Buscar" onSearchAction={()=>navigate('/contactos')} name={ctx?.role||'Usuario'} role="" initials={(ctx?.role||'U').slice(0,2).toUpperCase()} onToggleTheme={()=>setTheme(theme==='light'?'dark':'light')} onLogout={logout} contentClassName="contact-detail-content">
