...
export default function OperationalSidebar({navigation,activeRoute,anaSubtitle='Cuando quieras, avanzamos paso a paso.',anaRoute='/ana',variant='default',className='',theme,ariaLabel}:Props){
 ...
 const anaCard=<button className="ops-ana" aria-label="Hablar con Ana" onClick={()=>navigate(anaRoute)}><img className="dir-help-avatar" style={{width:44,height:44,borderRadius:'50%',objectFit:'cover',objectPosition:'center',background:'#fff'}} src={anaAvatar} alt="Ana"/><span><strong>Ana está contigo</strong><small>{anaSubtitle}</small></span></button>;
