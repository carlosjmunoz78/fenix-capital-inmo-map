...
export default function BankCreateShell(){
 ...
 function prepare(){
  const q=new URLSearchParams({mode:'do',resource:'banco',intent:'nuevo',nombre:nombre.trim(),direccion:direccion.trim(),localidad:localidad.trim(),provincia:provincia.trim(),perfil,financiacion_100:cien,doble_garantia:doble});
  telefonos.map(x=>x.trim()).filter(Boolean).forEach(x=>q.append('telefono',x));
  emails.map(x=>x.trim()).filter(Boolean).forEach(x=>q.append('email',x));
 ...
 return <OperationalShellFrame theme={theme} navigation={nav.length?nav:fallbackNav} activeRoute="/bancos" query={globalQuery} onQueryChange={setGlobalQuery} searchPlaceholder="Buscar en toda la app..." searchActionLabel="Buscar" onSearchAction={globalSearch} name={ctx?.role||'Usuario'} role="" initials={(ctx?.role||'U').slice(0,2).toUpperCase()} onToggleTheme={()=>setTheme(theme==='light'?'dark':'light')} onLogout={logout}>
  ...
  <div className="ops-title"><div><span className="ops-icon"><Landmark size={20}/></span><div><h1>Nuevo banco</h1><p>Datos generales, contacto y criterios principales de la entidad.</p></div></div></div>
  <section className="inmo-ana-hero"><div className="inmo-ana-photo"><img src={anaVertical} alt="Ana"/></div><div className="inmo-ana-body"><span>ANA · NUEVO BANCO</span><h2>Vamos a preparar la ficha correctamente</h2><p>Primero registramos únicamente lo que sabemos. Puedes añadir todos los teléfonos y correos que tenga la entidad; ninguno queda limitado a un único contacto.</p><div className="inmo-next"><button onClick={()=>document.getElementById('nueva-ficha-banco')?.scrollIntoView({behavior:'smooth'})}><b>1</b><strong>Completar ficha</strong><small>Ir a datos →</small></button><button onClick={()=>navigate('/bancos/contactos')}><b>2</b><strong>Revisar contactos</strong><small>Abrir contactos →</small></button><button onClick={()=>navigate('/ana?mode=help&resource=banco&intent=nuevo')}><b>3</b><strong>Ayúdame</strong><small>Preparar con Ana →</small></button></div></div></section>
  <section id="nueva-ficha-banco" className="ops-table-card" style={{padding:20,display:'grid',gap:12}}>
   <label>Nombre del banco<input value={nombre} onChange={e=>setNombre(e.target.value)} placeholder="Entidad"/></label>
   <label>Dirección<input value={direccion} onChange={e=>setDireccion(e.target.value)} placeholder="Calle, número, oficina o sucursal"/></label>
   <label>Localidad<input value={localidad} onChange={e=>setLocalidad(e.target.value)} placeholder="Localidad"/></label>
   <label>Provincia<input value={provincia} onChange={e=>setProvincia(e.target.value)} placeholder="Provincia"/></label>
   <div className="multi-contact-group"><div className="multi-contact-head"><strong>Teléfonos</strong><button type="button" onClick={()=>setTelefonos(v=>[...v,''])}><Plus size={15}/> Añadir teléfono</button></div>{telefonos.map((value,index)=><label key={`tel-${index}`}>Teléfono {index+1}<span className="multi-contact-row"><input value={value} onChange={e=>setTelefonos(v=>updateList(v,index,e.target.value))} inputMode="tel" placeholder="Teléfono"/>{telefonos.length>1&&<button type="button" aria-label={`Quitar teléfono ${index+1}`} onClick={()=>setTelefonos(v=>v.filter((_,i)=>i!==index))}><X size={15}/></button>}</span></label>)}</div>
   <div className="multi-contact-group"><div className="multi-contact-head"><strong>Correos</strong><button type="button" onClick={()=>setEmails(v=>[...v,''])}><Plus size={15}/> Añadir correo</button></div>{emails.map((value,index)=><label key={`mail-${index}`}>Correo {index+1}<span className="multi-contact-row"><input value={value} onChange={e=>setEmails(v=>updateList(v,index,e.target.value))} type="email" placeholder="correo@entidad.es"/>{emails.length>1&&<button type="button" aria-label={`Quitar correo ${index+1}`} onClick={()=>setEmails(v=>v.filter((_,i)=>i!==index))}><X size={15}/></button>}</span></label>)}</div>
   <label>Perfil principal<select value={perfil} onChange={e=>setPerfil(e.target.value)}><option value="">Sin clasificar</option><option>General</option><option>Jóvenes</option><option>Funcionarios</option><option>Autónomos</option><option>Segunda vivienda</option><option>Operaciones especiales</option></select></label><label>Financiación 100%<select value={cien} onChange={e=>setCien(e.target.value)}><option>No informado</option><option>Sí</option><option>No</option></select></label><label>Doble garantía<select value={doble} onChange={e=>setDoble(e.target.value)}><option>No informado</option><option>Sí</option><option>No</option></select></label><button className="primary" disabled={!nombre.trim()} onClick={prepare}>Preparar alta con Ana</button>
  </section>
