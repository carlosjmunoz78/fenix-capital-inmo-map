...
function canonicalId(v:string){return v.replace(/^notion\|/i,'').trim();}
...
export default function InmobiliariaDetailShell(){
 const location=useLocation(),navigate=useNavigate();const match=location.pathname.match(/^\/inmobiliarias\/([^/]+)$/);const routeId=match?.[1]?decodeURIComponent(match[1]):'';const id=canonicalId(routeId);const active=Boolean(match&&validDetailId(routeId)&&id);
