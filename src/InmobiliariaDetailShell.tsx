...
function validDetailId(v:string){const raw=v.trim().toLowerCase();return Boolean(raw&&raw!=='nuevo'&&raw!=='nueva');}
...
async function actionApi(code:string,expectedVersion:number,changes:Record<string,unknown>){
 const{data,error}=await supabase.rpc('fenix_prod_inmo_followup_update_v1',{
  ...
  p_proximo_contacto:typeof changes.proximo_contacto_b2b==='string'&&changes.proximo_contacto_b2b?changes.proximo_contacto_b2b:null
 });
 const body=(data??{ok:false,error:error?.message}) as any;
 return{status:error?500:Number(body?.status||200),data:body};
}

export default function InmobiliariaDetailShell(){
 const location=useLocation(),navigate=useNavigate();const match=location.pathname.match(/^\/inmobiliarias\/([^/]+)$/);const code=match?.[1]?decodeURIComponent(match[1]):'';const active=Boolean(match&&validDetailId(code));
 ...
 const zone=text(row||undefined,['zona','zona_operativa','zone_code','zona_actuacion','zona_actuación'])||'No disponible';
 ...
 const version=Number(value(row||undefined,['version'])||0);
 ...
 function prepareCorrection(){if(!correction.trim()||!reason.trim())return;const q=new URLSearchParams({mode:'help',resource:'inmobiliaria',inmobiliaria_id:code,correction:correction.trim(),reason:reason.trim()});navigate(`/ana?${q.toString()}`);}
 async function save(){if(!preview){setPreview(true);return;}if(!notes.trim()&&!nextContact)return;if(!version){setSaveMsg('No se puede guardar sin una versión canónica válida. Recarga la ficha.');return;}setSaving(true);setSaveMsg('');const changes:Record<string,unknown>={};if(notes.trim())changes.notas=notes.trim();if(nextContact)changes.proximo_contacto_b2b=nextContact;const r=await actionApi(code,version,changes);setSaving(false);if(r.status===200&&r.data?.ok){setSaveMsg('Seguimiento B2B guardado en la fuente canónica PROD.');setNotes('');setNextContact('');setPreview(false);await load();}else if(r.status===403)setSaveMsg('Tu perfil no puede modificar esta inmobiliaria o queda fuera de tu zona/cartera.');else if(r.status===409)setSaveMsg('La ficha cambió mientras la editabas. Se ha recargado para evitar sobrescribir datos.');else setSaveMsg(`No se pudo guardar el seguimiento (${r.data?.error||r.status}).`);if(r.status===409)await load();}
