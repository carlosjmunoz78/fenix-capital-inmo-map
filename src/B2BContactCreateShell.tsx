...
import {ArrowLeft,LogOut,Moon,Save,Sun,UserPlus} from 'lucide-react';
...
export default function B2BContactCreateShell(){
 ...
 return <div className="ops-root" data-theme={theme} style={{zIndex:7000}}>
  <aside className="ops-side"><button className="ops-brand" onClick={()=>navigate('/inicio')}><img src={fenixLogo} alt=""/><strong>FÉNIX CAPITAL</strong></button><nav><button onClick={()=>navigate(`/inmobiliarias/${encodeURIComponent(inmoId)}`)}><ArrowLeft size={15}/> Volver a la inmobiliaria</button></nav><button className="ops-ana" onClick={()=>navigate('/ana')}><img src={anaAvatar} alt="Ana"/><span><strong>Ana está contigo</strong><small>Este contacto queda ligado a la inmobiliaria actual.</small></span></button></aside>
  <main className="ops-main"><header className="ops-top"><strong>Nuevo contacto B2B</strong><div className="ops-top-actions"><button onClick={()=>setTheme(theme==='light'?'dark':'light')} aria-label="Cambiar tema">{theme==='light'?<Moon size={17}/>:<Sun size={17}/>} {theme==='light'?'Oscuro':'Claro'}</button><div className="ops-profile"><strong>{ctx?.role||'Usuario'}</strong></div><button onClick={logout} aria-label="Cerrar sesión"><LogOut size={17}/></button></div></header>
   <section className="ops-content"><div className="ops-title"><div><span className="ops-icon"><UserPlus size={20}/></span><div><h1>Nuevo contacto de inmobiliaria</h1><p>Contacto B2B separado de clientes hipotecarios.</p></div></div><span className="ops-live ok">PRE-PROD</span></div>
