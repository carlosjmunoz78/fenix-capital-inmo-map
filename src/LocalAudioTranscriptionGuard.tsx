...
export default function LocalAudioTranscriptionGuard(){
 ...
 const[authenticated,setAuthenticated]=useState(false),[authReady,setAuthReady]=useState(false);
 const[open,setOpen]=useState(false),[busy,setBusy]=useState(false),[status,setStatus]=useState(''),[text,setText]=useState('');
 const[voiceMenuHost,setVoiceMenuHost]=useState<HTMLElement|null>(null);
 const input=useRef<HTMLInputElement|null>(null);
 useEffect(()=>{let alive=true;supabase.auth.getSession().then(({data})=>{if(alive){setAuthenticated(Boolean(data.session));setAuthReady(true)}});const{data:{subscription}}=supabase.auth.onAuthStateChange((_event,session)=>{setAuthenticated(Boolean(session));setAuthReady(true);if(!session)setOpen(false)});return()=>{alive=false;subscription.unsubscribe()};},[]);
 useEffect(()=>{
  if(!authenticated)return;
  const sync=()=>setVoiceMenuHost(document.querySelector<HTMLElement>('.fenix-voice-menu'));
 ...
 async function choose(e:ChangeEvent<HTMLInputElement>){
  ...
  try{
   if(ctx){setStatus('Guardando audio original…');await saveEvidence(file,ctx,'audio_conversacion');}
   const transcript=await transcribeLocal(file,setStatus);if(!transcript)throw new Error('empty_transcript');
   setText(transcript);
   if(ctx){setStatus('Guardando transcripción vinculada…');const txt=new File([transcript],`${file.name.replace(/\.[^.]+$/,'')}-transcripcion.txt`,{type:'text/plain'});await saveEvidence(txt,ctx,'texto_conversacion');setStatus('Audio y transcripción guardados y vinculados.');}
   else setStatus('Transcripción local completada. Puedes copiar el texto; no se ha escrito nada en el CRM.');
  }catch(e){setStatus(e instanceof Error&&e.message==='file_too_large'?'El audio supera el tamaño máximo permitido.':ctx?'No se pudo completar la transcripción. El audio original no se borra si ya quedó guardado.':'No se pudo completar la transcripción local.');}
  finally{setBusy(false);}
 }
 if(!authReady||!authenticated)return null;
 const scopeText=ctx?`Se guarda el audio original y la transcripción en ${ctx.label}.`:'Modo general: transcribe localmente sin escribir en el CRM. En una ficha compatible, además quedará vinculado al registro.';
 const menuAction=voiceMenuHost?createPortal(<button type="button" data-testid="voice-upload-audio" onClick={()=>setOpen(true)}><FileAudio size={16}/> Subir audio y transcribir</button>,voiceMenuHost):null;
 return <>{menuAction}{open&&createPortal(<div style={{position:'fixed',inset:0,zIndex:10010,background:'rgba(20,16,24,.45)',display:'grid',placeItems:'center',padding:18}}><section style={{width:'min(620px,100%)',maxHeight:'86vh',overflow:'auto',background:'var(--panel,#fff)',color:'var(--text,#222)',border:'2px solid #870064',borderRadius:16,padding:18,display:'grid',gap:14}} aria-label="Transcribir audio"><div style={{display:'flex',justifyContent:'space-between',gap:12}}><div><strong>Audio → texto</strong><div style={{fontSize:12,marginTop:4}}>{scopeText} La transcripción se ejecuta localmente, sin API de pago.</div></div><button type="button" onClick={()=>setOpen(false)} disabled={busy} aria-label="Cerrar"><X size={16}/></button></div><button type="button" disabled={busy} onClick={()=>input.current?.click()} style={{padding:12,borderRadius:12,fontWeight:800,cursor:busy?'wait':'pointer'}}>{busy?<><LoaderCircle size={16}/> Procesando…</>:'Elegir audio'}</button><input ref={input} type="file" accept={AUDIO_EXT} onChange={e=>void choose(e)} style={{display:'none'}}/>{status&&<strong style={{fontSize:12}}>{status}</strong>}{text&&<><textarea readOnly value={text} aria-label="Transcripción" style={{minHeight:150,width:'100%',padding:12,borderRadius:10}}/><button type="button" onClick={()=>void navigator.clipboard?.writeText(text)} style={{padding:10,borderRadius:10,fontWeight:800}}>Copiar texto</button></>}<small>La primera transcripción descarga y almacena en caché el modelo local; las siguientes reutilizan esa caché.</small></section></div>,document.body)}</>;
}
