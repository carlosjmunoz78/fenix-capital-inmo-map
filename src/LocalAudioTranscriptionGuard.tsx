...
export default function LocalAudioTranscriptionGuard(){
 ...
 async function choose(e:ChangeEvent<HTMLInputElement>){
  ...
  try{
   setStatus('Guardando audio original…');await saveEvidence(file,ctx,'audio_conversacion');
   ...
   const txt=new File([transcript],`${file.name.replace(/\.[^.]+$/,'')}-transcripcion.txt`,{type:'text/plain'});await saveEvidence(txt,ctx,'texto_conversacion');
