...
function personForFile(name:string,people:Person[]){const n=` ${words(name)} `;const ranked=people.map(person=>({person,score:Math.max(0,...personAliases(person).filter(alias=>n.includes(` ${alias} `)).map(alias=>alias.length))})).filter(x=>x.score>0).sort((a,b)=>b.score-a.score);return ranked.length&&(!ranked[1]||ranked[0].score>ranked[1].score)?ranked[0].person:null;}
function fileRule(name:string,people:Person[]){const n=words(name),direct=DOC_RULES.find(rule=>rule.re.test(n));if(direct)return direct;const person=personForFile(name,people);if(person&&/\bcontrato\b/i.test(n)&&!/(arras|compraventa|reserva|alquiler|arrendamiento)/i.test(n))return{type:'Contrato laboral',family:'employment_contract',re:/contrato/i,personRequired:true};return null;}
...
export default function ContextEvidenceUpload(){
 ...
 async function routesFor(files:File[],target:OriginCtx){
  ...
  return files.map(file=>{const rule=fileRule(file.name,people);if(!rule)throw new Error(`No reconozco el tipo por el nombre de archivo: ${file.name}. Renómbralo indicando DNI, nómina, vida laboral, movimientos, IRPF, CIRBE, contrato + persona, arras, nota simple, catastro o tasación.`);const person=rule.personRequired?personForFile(file.name,people):null;if(rule.personRequired&&!person)throw new Error(`No puedo asignar ${file.name} a una única persona. Incluye en el nombre del archivo el nombre de la persona tal como aparece en Intervinientes.`);return{file,type:rule.type,family:rule.family,person:person?.comprador||[person?.nombre,person?.apellidos].filter(Boolean).join(' ')||'Expediente',contactCode:person?.id||''};});
 }
