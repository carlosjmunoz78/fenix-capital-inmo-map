...
function rowsFrom(data:unknown):Row[]{if(!data||typeof data!=='object')return[];const d=data as Record<string,unknown>;if(Array.isArray(d.items))return d.items as Row[];return Array.isArray(d.tasaciones)?d.tasaciones as Row[]:[];}
function text(row:Row,keys:string[]){for(const k of keys){const v=row[k];if(typeof v==='string'&&v.trim())return v.trim();}return'';}
function idOf(r:Row){return text(r,['tasacion_code','id','tasacion_id','appraisal_code','code']);}
function addressOf(r:Row){return text(r,['direccion','dirección','inmueble','domicilio','direccion_inmueble','cliente_alias','expediente_code'])||'Tasación sin inmueble visible';}
function stateOf(r:Row){return text(r,['estado','estado_tasacion','estado_tasación'])||'Sin estado';}
function appraiserOf(r:Row){return text(r,['tasador','tasadora','empresa_tasadora','proveedor','assigned_by'])||'No disponible';}
function dateOf(r:Row){return text(r,['fecha_informe','fecha_visita','fecha','fecha_tasacion','fecha_tasación','updated_at'])||'No disponible';}
...
function hasReport(r:Row){return Boolean(text(r,['informe_document_code','pdf','url_pdf','informe','documento','archivo']))||/informe recibido|enviada banco|finaliz|complet/i.test(stateOf(r));}
...
export default function TasacionesShell(){
 ...
 const sourceLabel=IS_PRODUCTION?'Fuente canónica Fénix':'Fuente canónica Notion';
 return <OperationalShellFrame className="tas-root" theme={theme} navigation={effectiveNav} activeRoute="/tasaciones" anaSubtitle="Cuando quieras, avanzamos paso a paso." anaRoute="/ana" query={query} onQueryChange={setQuery} searchPlaceholder="Buscar dirección, tasador o estado..." searchActionLabel="Buscar" name={role} role="" initials={role.slice(0,2).toUpperCase()} onToggleTheme={()=>setTheme(theme==='light'?'dark':'light')} onLogout={logout} contentClassName="tas-content">
    ...
    {!loading&&status===200&&<section className="tas-kpis"><article><small>TASACIONES</small><strong>{rows.length}</strong><span>Visibles en tu ámbito</span></article><article><small>CON INFORME</small><strong>{withReport}</strong><span>Informe disponible o finalizado</span></article><article><small>PENDIENTES</small><strong>{pendingCount}</strong><span>Solicitadas, en visita o en curso</span></article></section>}
    <div className="tas-title"><div><small>TASACIÓN Y VALORACIÓN</small><h1>Tasaciones</h1><p>{loading?'Cargando datos autorizados…':status===200?`${rows.length} tasaciones visibles en tu ámbito autorizado.`:'Lectura canónica según permisos.'}</p></div><span className={status===200&&!loading?'ops-live ok':'ops-live'}>{loading?'Cargando…':status===200?'Datos vivos':IS_PRODUCTION?'Sin conexión':'PRE-PROD'}</span></div>
