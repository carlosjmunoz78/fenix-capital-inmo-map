...
function normalizeNav(data:unknown):NavItem[]{
  ...
  return items.map(x=>{
    ...
    if(x&&typeof x==='object'){
      const o=x as Record<string,unknown>; if(typeof o.route==='string')return {label:typeof o.label==='string'?o.label:o.route.replace(/^\//,''),route:o.route,resource:typeof o.resource==='string'?o.resource:undefined};
    }
    return null;
...
function rowsFrom(data:unknown):AnyRow[]{
  ...
  for(const key of ['items','expedientes','bancos','inmobiliarias','documentos','tareas','tasaciones','firmas','results','reports']) if(Array.isArray(d[key]))return d[key] as AnyRow[];
  return [];
}
function prettyKey(k:string){return k.replaceAll('_',' ').replace(/\b\w/g,m=>m.toUpperCase());}
function prettyValue(v:unknown){if(v===null||v===undefined||v==='')return '—';if(typeof v==='boolean')return v?'Sí':'No';if(typeof v==='object')return JSON.stringify(v);return String(v);}
async function fetchReports(){
  ...
  let data:unknown=null;try{data=await r.json()}catch{data=null} return {status:r.status,data};
}
...
export default function OperationalShellV2(){
  ...
  useEffect(()=>{let alive=true;supabase.auth.getSession().then(({data})=>{if(alive){setLogged(Boolean(data.session));setSessionReady(true)}});const {data:{subscription}}=supabase.auth.onAuthStateChange((_e,s)=>{setLogged(Boolean(s));setSessionReady(true)});return()=>{alive=false;subscription.unsubscribe()};},[]);
  useEffect(()=>{document.documentElement.dataset.theme=theme;sessionStorage.setItem('fenix-theme',theme);},[theme]);
  useEffect(()=>{if(!logged||!module)return;Promise.all([fetchAppApi<Ctx>('/session/context'),fetchAppApi<unknown>('/navigation')]).then(([c,n])=>{setCtx(c.status===200?c.data:null);setNav(n.status===200?normalizeNav(n.data):[]);});},[logged,module?.title]);
  useEffect(()=>{
    if(!logged||!module)return;let alive=true;
    async function load(){setLoading(true);setRows([]);setMessage('');setStatus(null);
      if(key==='/economia'){setMessage('Economía queda reservada a la fase de Dirección. No se muestran cifras inventadas.');setStatus(503);setLoading(false);return;}
      if(key==='/buscar'){const q=new URLSearchParams(location.search).get('q')?.trim()||'';if(q.length<2){setMessage('Escribe al menos 2 caracteres para buscar.');setStatus(200);setLoading(false);return;}const r=await fetchAppApi<unknown>(`/search?q=${encodeURIComponent(q)}`);if(!alive)return;setStatus(r.status);setRows(rowsFrom(r.data));if(r.status!==200)setMessage('No se pudo completar la búsqueda.');setLoading(false);return;}
      if(key==='/informes'){const r=await fetchReports();if(!alive)return;setStatus(r.status);setRows(rowsFrom(r.data));if(r.status!==200)setMessage('No se pudieron cargar los informes.');setLoading(false);return;}
      if(!module.endpoint){setMessage('Módulo pendiente de conexión PRE-PROD.');setStatus(503);setLoading(false);return;}
      const r=await fetchAppApi<unknown>(module.endpoint);if(!alive)return;setStatus(r.status);setRows(rowsFrom(r.data));if(r.status===403)setMessage('Tu perfil no tiene acceso a este módulo.');else if(r.status!==200)setMessage('No se pudo cargar el módulo.');setLoading(false);
    }
  ...
  const columns=useMemo(()=>{const first=rows[0];if(!first)return [];return Object.keys(first).filter(k=>!['id','synthetic','updated_at','created_at','owner_actor_code'].includes(k)).slice(0,6);},[rows]);
  if(!sessionReady||!logged||!module)return null;const Icon=module.icon;
