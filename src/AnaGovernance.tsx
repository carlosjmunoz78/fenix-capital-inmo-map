...
export default function AnaGovernance(){
  ...
  useEffect(()=>{
    ...
    const preparedReason=params.get('reason')?.trim()||'';
    const preparedRule=params.get('rule')?.trim()||'';
    if(preparedSuggestion)setSuggestion(preparedSuggestion);
    if(preparedReason)setReason(preparedReason);
    if(preparedRule)setRule(preparedRule);
  },[active,params]);
  ...
  async function submit(e:FormEvent){e.preventDefault();setMessage('');if(reason.trim().length<2||suggestion.trim().length<2){setMessage('Indica qué sugirió Ana y por qué no debe hacerse así.');return;}setSaving(true);const payload:Record<string,unknown>={scope_type:scopeType,ana_suggestion:suggestion,user_reason:reason,proposed_rule:rule};if(scopeCode)payload.scope_code=scopeCode;const r=await fetchAppApi('/ana/correcciones',{method:'POST',body:JSON.stringify(payload)});setSaving(false);if(r.status===201){setSuggestion('');setReason('');setRule('');setMessage('Corrección enviada para revisión de Dirección.');await load();}else setMessage('No se pudo guardar la corrección.');}
  ...
  return <div className="ops-root" data-theme={theme}>
    ...
    <main className="ops-main"><header className="ops-top"><div className="ops-profile"><strong>Gobierno de Ana · {ctx?.role||'Usuario'}</strong></div><div className="ops-top-actions"><button onClick={()=>setTheme(theme==='light'?'dark':'light')} aria-label="Cambiar tema">{theme==='light'?<Moon size={17}/>:<Sun size={17}/>} {theme==='light'?'Oscuro':'Claro'}</button><button onClick={logout} aria-label="Cerrar sesión"><LogOut size={17}/></button></div></header>
      <section className="ops-content"><div className="ops-title"><div><img src={anaAvatar} alt="Ana" style={{width:48,height:48,borderRadius:'50%'}}/><div><h1>Hablar con Ana</h1><p>Una acción cada vez. Las correcciones no se convierten en norma sin revisión.</p></div></div><span className="ops-live ok">Gobierno activo</span></div>
        ...
        <form className="ops-message" onSubmit={submit} style={{display:'grid',gap:10}}><strong>Corregir una sugerencia de Ana</strong>{scopeType!=='general'&&<small>Contexto: {scopeType}{scopeCode?` · ${scopeCode}`:''}</small>}<label>¿Qué sugirió Ana?<textarea value={suggestion} onChange={e=>setSuggestion(e.target.value)} rows={2} style={{width:'100%'}}/></label><label>¿Por qué no debe hacerse así?<textarea value={reason} onChange={e=>setReason(e.target.value)} rows={3} style={{width:'100%'}}/></label><label>Regla propuesta para próximas veces (opcional)<textarea value={rule} onChange={e=>setRule(e.target.value)} rows={2} style={{width:'100%'}}/></label><button className="primary" disabled={saving}>{saving?'Guardando…':'Enviar a revisión'}</button></form>
