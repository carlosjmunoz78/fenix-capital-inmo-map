...
function savedAlias(item:ExpRow|null){return String(item?.cliente_alias||'').trim();}
function personName(p:Person){return String(p.nombre||'').trim()||[p.nombre,p.apellidos].filter(Boolean).join(' ').trim()||String(p.comprador||'').trim();}
...
export default function ExpedienteRenameGuard(){
 ...
 const visibleName=useMemo(()=>savedAlias(item)||fallbackName(item,people,code),[item,people,code]);
 async function load(){if(!code)return;const detail=await fetchNotionRuntime<any>(`/expedientes/${encodeURIComponent(code)}`);if(detail.status!==200){setItem(null);setPeople([]);return;}const row=(detail.data?.expediente??detail.data?.item??null) as ExpRow|null;setItem(row);let nextPeople:Person[]=[];if(isNotionId(code)){const pr=await fetchNotionRuntime<PeopleResponse>(`/expedientes/${encodeURIComponent(code)}/compradores`);if(pr.status===200&&Array.isArray(pr.data?.items))nextPeople=pr.data!.items!;}setPeople(nextPeople);setName(savedAlias(row)||fallbackName(row,nextPeople,code));}
