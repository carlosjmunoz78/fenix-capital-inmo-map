...
export default function ExpedienteRenameGuard(){
 ...
 useEffect(()=>{if(!code||pathname==='/expedientes/nuevo'){setHost(null);return;}const mount=()=>{const title=document.querySelector<HTMLElement>('.detail-exp-root .detail-master-title');if(!title)return;applyVisibleTitle(title.querySelector<HTMLElement>('h1'),visibleName,code);let node=title.querySelector<HTMLElement>(':scope > .exp-rename-host');if(!node){node=document.createElement('div');node.className='exp-rename-host';title.appendChild(node);}setHost(current=>current===node?current:node);};mount();const observer=new MutationObserver(mount);observer.observe(document.body,{childList:true,subtree:true});return()=>{observer.disconnect();document.querySelectorAll('.exp-rename-host').forEach(x=>x.remove());setHost(null);};},[code,pathname,visibleName]);
