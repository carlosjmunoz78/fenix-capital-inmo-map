...
import {runOperationalRecordAction,type OperationalResource} from './operationalRecordActions';
...
type Resource=OperationalResource;
...
export default function OperationalRecordDetail(){
 ...
 async function load(){if(!active||!def||(!IS_PRODUCTION&&!isNotionId(id))){setStatus(404);setMsg('No se ha encontrado la ficha canónica.');return;}setLoading(true);setMsg('');const r=await fetchNotionRuntime<any>(`${def.runtime}/${encodeURIComponent(id)}`);setStatus(r.status);setData(r.data);setLoading(false);if(r.status===403)setMsg('Tu perfil no puede abrir este registro.');else if(r.status===404)setMsg('No se ha encontrado el registro.');else if(r.status!==200)setMsg('No se pudo cargar el registro.');}
 ...
 async function save(){if(!Object.keys(clean).length)return;setBusy(true);setSaveMsg('');const r=await runOperationalRecordAction(def.resource,id,item,clean);setBusy(false);if(r.status===200){setSaveMsg(IS_PRODUCTION?'Cambios guardados y auditados en la fuente canónica PROD.':'Cambios guardados y auditados en Notion TEST.');setChanges({});setPreview(false);await load();}else if(r.status===403)setSaveMsg('Tu perfil no puede modificar este registro.');else if(r.status===409&&r.data?.error==='CANONICAL_WRITE_UNAVAILABLE')setSaveMsg('Este cambio no tiene todavía un contrato de escritura canónico seguro en PROD. No se ha modificado ningún dato.');else setSaveMsg(`No se pudo guardar (${r.data?.error||r.status}).`);}
 const sourceLabel=IS_PRODUCTION?'Fuente canónica PROD · acciones limitadas por rol y propietario.':'Fuente de prueba Notion · acciones limitadas por rol y propietario.';
 const liveLabel=loading?'Cargando…':status===200?(IS_PRODUCTION?'PROD canónico':'Notion TEST'):'Sin datos';
 return <div className="ops-root" style={{zIndex:5400}} data-theme={(sessionStorage.getItem('fenix-theme')||'light')}>
  ...
  <main className="ops-main"><header className="ops-top"><strong>{def.title}</strong></header><section className="ops-content"><button className="inmo-detail-back" onClick={()=>navigate(def.listRoute)}><ArrowLeft size={15}/> Volver a {def.title==='Tarea'?'Agenda':def.title==='Documento'?'Documentación':def.title==='Tasación'?'Tasaciones':'Firmas'}</button>
   <div className="ops-title"><div><span className="ops-icon"><Icon size={20}/></span><div><h1>{display}</h1><p>{sourceLabel}</p></div></div><span className={status===200?'ops-live ok':'ops-live'}>{liveLabel}</span></div>
   ...
   {item&&<><div className="ops-message" style={{display:'grid',gridTemplateColumns:'repeat(2,minmax(0,1fr))',gap:10}}>{fields.map(k=><div key={k}><small>{pretty(k)}</small><div><strong>{val(item[k])}</strong></div></div>)}</div>
    <div className="ops-table-card"><div className="ops-table-head"><strong>Acción contextual</strong><span>{ctx?.role||'Usuario'} · escritura auditada</span></div><div className="ops-message" style={{display:'grid',gap:10}}>
      ...
      {preview&&<div className="ops-message"><strong>Vista previa</strong><div>{Object.entries(clean).map(([k,v])=><div key={k}>{pretty(k)}: {val(v)}</div>)}</div><small>{IS_PRODUCTION?'Confirma para escribir únicamente estos cambios mediante el contrato canónico PROD.':'Confirma para escribir únicamente estos cambios en Notion TEST.'}</small></div>}
