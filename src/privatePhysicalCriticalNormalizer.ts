...
function moneyNear(text:string,labels:RegExp[],radius=4){const lines=ls(text);for(let i=0;i<lines.length;i++)for(const label of labels){if(!label.test(lines[i]))continue;for(let d=0;d<=radius;d++){for(const j of [i+d,i-d]){if(j<0||j>=lines.length)continue;const n=parseMoney(lines[j]);if(n!==null)return n;}}}return null;}
