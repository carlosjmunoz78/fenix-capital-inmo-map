...
function section(text:string,start:RegExp,stops:RegExp[],max=1800){const lines=ls(text);const i=lines.findIndex(x=>start.test(x));if(i<0)return'';const out:string[]=[];for(let j=i+1;j<lines.length;j++){if(stops.some(s=>s.test(lines[j])))break;out.push(lines[j]);if(out.join(' | ').length>=max)break;}return clean(out.join(' | '),max);}
...
function moneyNear(text:string,labels:RegExp[]){const lines=ls(text);for(let i=0;i<lines.length;i++)for(const label of labels){if(!label.test(lines[i]))continue;const n=parseMoney(lines[i]);if(n!==null)return n;if(lines[i+1]){const next=parseMoney(lines[i+1]);if(next!==null)return next;}}return null;}
function knownEntity(text:string){const lines=ls(text).slice(0,80);const rx=/\b(BBVA|BANCO BILBAO VIZCAYA ARGENTARIA|CAIXABANK|BANCO SANTANDER|SANTANDER|BANCO SABADELL|SABADELL|UNICAJA(?: BANCO)?|ABANCA|BANKINTER|CAJAMAR|CAJA RURAL(?: DE [A-ZÁÉÍÓÚÜÑ .'-]{3,80})?|ING(?: BANK)?|OPENBANK|CETELEM|COFIDIS|WIZINK|YOUNITED(?: CREDIT)?|CARREFOUR(?: FINANZAS)?)\b/i;for(const line of lines){const m=line.match(rx);if(m?.[1])return clean(m[1],120);}return'';}
...
function normalizeNota(text:string,f:ExtractedFields){
 ...
 const desc=section(text,/^(?:DESCRIPCI[ÓO]N(?: DE LA FINCA)?|DESCRIPCI[ÓO]N FINCA|DATOS DE LA FINCA)\b/i,[/^TITULAR(?:IDAD|IDADES|ES)?\b/i,/^CARGAS\b/i,/^ASIENTOS\b/i],2400);if(desc)set(f,'descripcion_finca',desc);
 const titulares=section(text,/^TITULAR(?:IDAD|IDADES|ES)?\b/i,[/^CARGAS\b/i,/^ASIENTOS\b/i,/^LIMITACIONES\b/i],1800);if(titulares)set(f,'titulares',titulares);
 const cargas=section(text,/^CARGAS(?: DE LA FINCA)?\b/i,[/^ASIENTOS\b/i,/^INFORMACI[ÓO]N\b/i,/^DOCUMENTOS\b/i,/^FECHA\b/i],2400);if(cargas){set(f,'cargas',cargas);if(/HIPOTECA/i.test(cargas))set(f,'hipotecas',cargas.match(/[^|]*HIPOTECA[^|]*/i)?.[0]||'Hipoteca consta en cargas');if(/EMBARGO/i.test(cargas))set(f,'embargos',cargas.match(/[^|]*EMBARGO[^|]*/i)?.[0]||'Embargo consta en cargas');}
...
function normalizeDebt(text:string,f:ExtractedFields){
 const entity=inlineOrNext(text,/\b(?:ENTIDAD|ACREEDOR|BANCO)\s*[:\-–—]?/i,180)||knownEntity(text);if(entity){set(f,'entidad',entity);set(f,'acreedor',entity);}
 const balance=moneyNear(text,[/CAPITAL\s+PENDIENTE/i,/SALDO\s+PENDIENTE/i,/PRINCIPAL\s+PENDIENTE/i,/DEUDA\s+PENDIENTE/i]);if(balance!==null){set(f,'capital_pendiente',balance);set(f,'saldo_pendiente',balance);}
 const cuota=moneyNear(text,[/^CUOTA\b/i,/IMPORTE\s+(?:DEL\s+)?RECIBO/i,/MENSUALIDAD/i,/CUOTA\s+MENSUAL/i]);if(cuota!==null)set(f,'cuota',cuota);
