...
import {fetchAppApi,fetchB2BActionsApi,IS_PRODUCTION,supabase} from './supabase';
...
export default function B2BContactDetailShell(){
 ...
 async function save(){if(!allowed||!preview||busy)return;setBusy(true);const next={nombre:nombre.trim(),apellidos:apellidos.trim(),cargo:cargo.trim(),email:email.trim(),telefono:telefono.trim()};const r=await fetchB2BActionsApi<Envelope>(`/contactos/${encodeURIComponent(id)}/update`,{method:'POST',body:JSON.stringify(next)});setBusy(false);if(r.status===200){setPreview(false);setData(prev=>prev?.item?{...prev,item:{...prev.item,...next,contacto:[next.nombre,next.apellidos].filter(Boolean).join(' ')||prev.item.contacto}}:prev);setMessage('Contacto B2B actualizado dentro de tu ámbito autorizado.');}else if(r.status===403)setMessage('No puedes modificar este contacto B2B.');else setMessage(`No se pudo actualizar (${r.data?.error||r.status}).`);}
 ...
 return <OperationalShellFrame className="b2b-contact-detail-root" theme={theme} navigation={effectiveNav} activeRoute="/inmobiliarias" anaSubtitle="Solo uso el contexto B2B autorizado." anaRoute={`/ana?mode=help&scope_type=contacto_b2b&scope_code=${encodeURIComponent(id)}`} query="" onQueryChange={()=>{}} searchPlaceholder="" name={ctx?.role||'Usuario'} role="" initials={(ctx?.role||'U').slice(0,2).toUpperCase()} onToggleTheme={()=>setTheme(theme==='light'?'dark':'light')} onLogout={logout} topbar={topbar}>
   <button className="inmo-detail-back" onClick={()=>inmo?.id?navigate(`/inmobiliarias/${encodeURIComponent(inmo.id)}`):navigate('/inmobiliarias')}><ArrowLeft size={15}/> Volver a la inmobiliaria</button><div className="ops-title"><div><span className="ops-icon"><UserRound size={20}/></span><div><h1>{data?.item?.contacto||'Contacto de inmobiliaria'}</h1><p>{inmo?.nombre||'Inmobiliaria'}{inmo?.localidad?` · ${inmo.localidad}`:''}</p></div></div><span className="ops-live ok">{IS_PRODUCTION?'OPERATIVO':'PRE-PROD'}</span></div>
    ...
    {loading&&<div className="ops-empty" data-testid="b2b-contact-loading"><strong>Cargando contacto B2B…</strong></div>}
    {!loading&&message&&<div className="ops-message">{message}</div>}
    {!loading&&status===200&&data?.item&&<form className="ops-message" onSubmit={e=>e.preventDefault()} style={{display:'grid',gap:12}}>
