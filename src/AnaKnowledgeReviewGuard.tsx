...
const labels:Record<DecisionKind,string>={regla:'Regla canónica',excepcion_precedente:'Excepción / precedente',contexto_caso:'Solo contexto',descartar:'Descartar'};
...
export default function AnaKnowledgeReviewGuard(){
 ...
 const content=useMemo(()=>{
  ...
  return <section className="ops-table-card" data-testid="ana-knowledge-review" style={{marginTop:18}}>
   ...
   <div style={{padding:'10px 14px 0'}}><small>Solo aparecen aportaciones que tu autoridad puede decidir. Hipotecas y Finanzas quedan reservadas a Belén; el resto respeta la autoridad del dominio.</small>{actor&&<small style={{display:'block',marginTop:4}}>Actor PRE-PROD: {actor}</small>}{blocked>0&&<div className="ops-message" style={{marginTop:10}}>{blocked} aportación{blocked===1?'':'es'} pendiente{blocked===1?'':'s'} queda{blocked===1?'':'n'} reservada{blocked===1?'':'s'} a otra autoridad.</div>}{message&&<div className="ops-message" style={{marginTop:10}}>{message}</div>}</div>
   {pending&&<div className="ops-message" data-testid="knowledge-decision-preview" style={{margin:'12px 14px'}}><strong>Revisar clasificación antes de aplicar</strong><div>Dominio: {pending.item.domain}</div><div>Autoridad: {pending.item.authority}</div><div>Clasificación: {labels[pending.kind]}</div><div>Comentario: {pending.comment||'Sin comentario'}</div>{(pending.kind==='regla'||pending.kind==='excepcion_precedente')&&<div>Texto aprobado: {pending.approvedRule}</div>}<small>{pending.kind==='regla'||pending.kind==='excepcion_precedente'?'Si confirmas, quedará aprobado en la base canónica. La reutilización efectiva por Ana depende del lector/runtime canónico.':'No se convertirá en regla canónica.'}</small><div style={{display:'flex',gap:8,marginTop:10,flexWrap:'wrap'}}><button type="button" onClick={()=>setPending(null)} disabled={saving}>Cancelar</button><button type="button" className="primary" onClick={()=>void confirm()} disabled={saving}>{saving?'Guardando…':'Confirmar clasificación'}</button></div></div>}
   <div className="ops-table-wrap"><table><thead><tr><th>Dominio</th><th>Conocimiento</th><th>Origen</th><th>Comentario</th><th>Texto aprobado</th><th>Clasificar</th></tr></thead><tbody>{!loading&&items.length===0?<tr><td colSpan={6}>No tienes conocimiento pendiente que revisar.</td></tr>:items.map(item=><tr key={item.id}><td><strong>{item.domain}</strong><br/><small>{item.authority}</small></td><td>{item.detail||'—'}{item.context?<><br/><small>{item.context}</small></>:null}</td><td>{item.source||'—'}{item.date?<><br/><small>{item.date}</small></>:null}</td><td><textarea aria-label={`Comentario revisión ${item.id}`} value={comments[item.id]||''} onChange={e=>setComments(v=>({...v,[item.id]:e.target.value}))} rows={3} style={{minWidth:190}} placeholder="Motivo, matiz o condición"/></td><td><textarea aria-label={`Texto aprobado ${item.id}`} value={rules[item.id]??item.detail} onChange={e=>setRules(v=>({...v,[item.id]:e.target.value}))} rows={3} style={{minWidth:210}}/></td><td><div style={{display:'grid',gap:6,minWidth:180}}>{(Object.keys(labels) as DecisionKind[]).map(kind=><button type="button" key={kind} onClick={()=>prepare(item,kind)} disabled={saving}>{labels[kind]}</button>)}</div></td></tr>)}</tbody></table></div>
  </section>;
