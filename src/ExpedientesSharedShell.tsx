...
export default function ExpedientesSharedShell(){
 ...
 const columns=useMemo(()=>{const first=visibleRows[0];if(!first)return[];return Object.keys(first).filter(k=>!['id','synthetic','updated_at','created_at','actualizado','owner_actor_code','fuente','destino'].includes(k)).slice(0,6);},[visibleRows]);
 ...
 return <OperationalShellFrame theme={theme} navigation={effectiveNav} activeRoute="/expedientes" query={globalQuery} onQueryChange={setGlobalQuery} searchPlaceholder="Buscar expediente, cliente, banco, inmobiliaria..." searchActionLabel="Buscar" onSearchAction={submitGlobalSearch} name={role} role={role} initials={role.slice(0,2).toUpperCase()} onToggleTheme={()=>setTheme(v=>v==='light'?'dark':'light')} onLogout={logout}>
  <div className="ops-title"><div><span className="ops-icon"><FolderOpen size={20}/></span><div><h1>Expedientes</h1><p>Cartera hipotecaria autorizada para tu perfil.</p></div></div><span className={status===200?'ops-live ok':'ops-live'}>{loading?'Cargando…':status===200?'Fuente canónica Notion':'PRE-PROD'}</span></div>
  ...
  <div className="ops-table-card exp-table"><div className="ops-table-head"><strong>Expedientes</strong><span>{visibleRows.length} registros</span></div><div className="ops-table-wrap"><table><thead><tr>{columns.map(column=><th key={column}><button onClick={()=>applySort(column)}>{prettyKey(column)} {sortKey===column?(sortDir==='asc'?'↑':'↓'):''}</button></th>)}</tr></thead><tbody>{visibleRows.map((row,index)=><tr key={firstString(row,['id','expediente_code','code','codigo'])||String(index)} onClick={()=>{const code=firstString(row,['expediente_code','code','codigo','id']);if(code)navigate(`/expedientes/${encodeURIComponent(code)}`)}}>{columns.map(column=><td key={column}>{prettyValue(row[column])}</td>)}</tr>)}</tbody></table></div></div>
