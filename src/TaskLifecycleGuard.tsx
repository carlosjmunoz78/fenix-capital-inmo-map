...
type TaskResponse={item?:{version?:number;estado?:string};ok?:boolean;status?:number;version?:number;error?:string};
...
export default function TaskLifecycleGuard(){
 ...
 const taskCode=match?.[1]?decodeURIComponent(match[1]):'';
 const active=Boolean(match)&&taskCode!=='nueva'&&taskCode!=='nuevo';
 ...
 useEffect(()=>{
  ...
  const place=()=>{
   const root=[...document.querySelectorAll<HTMLElement>('.ops-root')].filter(el=>{const r=el.getBoundingClientRect();return r.width>0&&r.height>0}).at(-1);
   const content=root?.querySelector<HTMLElement>('.ops-content');const card=content?.querySelector<HTMLElement>('.ops-table-card');if(!content||!card)return;
 ...
 return createPortal(<section data-testid="task-lifecycle" style={{marginTop:14,border:'1px solid var(--border)',borderRadius:16,padding:16,background:'var(--card)',display:'grid',gap:12}}>
  ...
  {open&&<div role="dialog" aria-modal="true" aria-label="Dar de baja tarea" style={{border:'1px solid var(--border)',borderRadius:14,padding:14,display:'grid',gap:10}}>
   ...
   <small>Tarea: {taskCode} · versión {version??(IS_PRODUCTION?'sin validar':'PRE-PROD')} · {IS_PRODUCTION?'PROD con escritura auditada':'PRE-PROD sin escritura PROD'}.</small>
  </div>}
 </section>,host)
}
