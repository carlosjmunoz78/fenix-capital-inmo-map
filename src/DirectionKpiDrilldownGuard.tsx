...
const SPECS:Record<string,Spec>={
 ...
 'honorarios-pendientes':{key:'honorarios-pendientes',title:'Honorarios pendientes este mes',route:'/economia?honorarios=pendientes&mes=actual&kpi=honorarios-pendientes',summary:'Este desglose solo se publicará cuando exista un contrato canónico de operaciones pendientes; no se reinterpretan movimientos contables como honorarios.',columns:[['expediente','Expediente'],['fecha_firma','Firma prevista'],['honorarios_fenix_base','Honorarios Fénix'],['comision_inmobiliaria','Comisión inmobiliaria'],['neto_pendiente_base','Neto pendiente']]}
};
...
async function load(key:string):Promise<{status:number;data:Payload|null}>{
 try{
  ...
  if(key==='honorarios-pendientes')return{status:204,data:null};
