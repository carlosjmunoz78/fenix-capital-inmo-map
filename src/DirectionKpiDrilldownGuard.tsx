...
function canonicalExp(r:Row){return {...r,expediente:r.expediente??r.expediente_code??r.id,cliente:r.cliente??r.cliente_alias,fase:r.fase??r.stage,proxima_accion:r.proxima_accion??r['próxima_accion'],riesgo:r.riesgo??r.semaforo??r['semáforo']}}
...
export default function DirectionKpiDrilldownGuard(){
 ...
 useEffect(()=>{if(!spec){setPortal(null);return}let cancelled=false,obs:MutationObserver|null=null;const attach=()=>{if(cancelled)return;const target=document.querySelector('.ops-content,.eco-content') as HTMLElement|null;if(target){setPortal(target);return}obs=new MutationObserver(attach);obs.observe(document.body,{childList:true,subtree:true})};attach();return()=>{cancelled=true;obs?.disconnect();setPortal(null)}},[spec?.key,location.pathname]);
