 ...
 return <div className={rootClass} data-theme={effectiveTheme} data-dir-theme={legacyDirectionTheme?effectiveTheme:undefined} data-mobile-nav={mobileNavOpen?'open':'closed'}>
  ...
  <main className={`ops-main ${mainClassName}`.trim()}>
   ...
   <section className={`ops-content ${contentClassName}`.trim()}>
    {children}
    {showSharedFooter&&quickLinks.length>0&&<section className="dir-quick ops-shared-quick" aria-label="Accesos rápidos"><h2>ACCESOS RÁPIDOS</h2><div className="dir-quick-grid">{quickLinks.map(({route,label,Icon})=><button key={route} type="button" onClick={()=>navigate(route)}><Icon/>{label}</button>)}</div></section>}
   </section>
  </main>
 </div>;
}
