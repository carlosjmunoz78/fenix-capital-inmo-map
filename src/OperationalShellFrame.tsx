...
export default function OperationalShellFrame({className='',theme,navigation,activeRoute,anaSubtitle,anaRoute,sidebarVariant='default',query,onQueryChange,searchPlaceholder,searchActionLabel,onSearchAction,name,role,avatarUrl='',initials,onToggleTheme,onLogout,topbar,mainClassName='',contentClassName='',legacyDirectionTheme=false,children}:Props){
 ...
 return <div className={`ops-root ${className}`.trim()} data-theme={effectiveTheme} data-dir-theme={legacyDirectionTheme?effectiveTheme:undefined}>
  <style>{`.ops-shared-footer{position:fixed;left:238px;right:0;bottom:0;z-index:7350;height:34px;display:flex;align-items:center;justify-content:center;border-top:1px solid var(--border,#e5e7eb);background:var(--panel,#fff);color:var(--muted,#667085);font-size:11px;letter-spacing:.08em;text-align:center}.ops-root[data-theme='dark'] .ops-shared-footer{background:#202023;border-color:#343438;color:#aaaab2}@media(max-width:900px){.ops-shared-footer{left:96px}}@media(max-width:650px){.ops-shared-footer{left:0;bottom:58px;height:30px;font-size:10px}}`}</style>
  ...
  <main className={`ops-main ${mainClassName}`.trim()}>
   ...
   <footer className="ops-shared-footer">FÉNIX CAPITAL · CEREBRO</footer>
  </main>
 </div>;
}
