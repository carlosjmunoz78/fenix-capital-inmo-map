...
export default function AnaKnowledgeBlock(){
  ...
  const [status,setStatus]=useState('Conectado con CEREBRO.');
  ...
  useEffect(()=>{
    document.querySelector('.ana-knowledge-mount')?.remove();
    setMount(null);
    if(location.pathname!=='/ana')return;
    const place=()=>{
      const roots=[...document.querySelectorAll('.ops-root')].filter(node=>{
      ...
      if(!content)return;
      let node=content.querySelector('.ana-knowledge-mount') as HTMLElement|null;
      if(!node){node=document.createElement('section');node.className='ana-knowledge-mount';content.appendChild(node)}
      setMount(node);
    };
    place();
    const observer=new MutationObserver(place);
    observer.observe(document.body,{childList:true,subtree:true});
    return()=>{observer.disconnect();document.querySelector('.ana-knowledge-mount')?.remove();setMount(null)};
  },[location.pathname]);
  ...
  async function submit(){
    ...
    const r=await fetchAnaKnowledgeApi<KnowledgeResult>('/knowledge',{method:'POST',body:JSON.stringify({detail,scope_type:'general',scope_code:null})});
    setSaving(false);
    if((r.status===200||r.status===201)&&r.data?.ok){
      setValue('');setPreview(false);setRecent(null);
      setStatus(r.data.reused?'Este conocimiento ya existía; CEREBRO no lo ha duplicado.':`Conocimiento registrado. Queda pendiente de revisión por ${r.data.authority||'Dirección'} antes de convertirse en un criterio reutilizable.`);
      return;
    }
