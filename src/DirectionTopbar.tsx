...
export default function DirectionTopbar({theme,search,profileName,initials,onSearchChange,onSearch,onNavigate,onToggleTheme,onLogout}:Props){
 ...
 return <header className="ops-top dir-topbar" style={{height:72,display:'flex',alignItems:'center',gap:18,padding:'0 26px'}}>
  <div className="ops-search dir-search" style={{height:39,flex:1,maxWidth:780}}><Search size={17}/><input value={search} onChange={e=>onSearchChange(e.target.value)} onKeyDown={e=>{if(e.key==='Enter')onSearch()}} placeholder="Buscar expediente, cliente, banco, inmobiliaria, contacto..."/><button type="button" className="dir-advanced" onClick={()=>onNavigate('/buscar')}><SlidersHorizontal size={15}/>Buscador avanzado</button></div>
  <div className="ops-top-actions dir-top-right" style={{marginLeft:'auto',display:'flex',alignItems:'center',gap:10}}>
