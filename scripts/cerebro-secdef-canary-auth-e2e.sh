...
# Canary must be healthy and must use a real authenticated identity for protected routes.
curl --fail --silent --show-error --max-time 30 \
  -H "apikey: ${PROD_PUBLISHABLE_KEY}" \
...
# Optional chat-message parity: only runs when a conversation code is supplied.
