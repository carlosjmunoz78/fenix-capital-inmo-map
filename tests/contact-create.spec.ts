...
test.describe('Fénix PRE-PROD · alta canónica de contactos',()=>{
 test('Dirección puede crear sin inventar asignación y con preview obligatorio',async({page},testInfo)=>{
  ...
  await page.route('**/functions/v1/fenix-contactos-unified-test',async r=>{posted=JSON.parse(r.request().postData()||'{}');return r.fulfill({status:201,contentType:'application/json',body:JSON.stringify({ok:true,id:'aaaaaaaa-1111-4111-8111-dddddddddddd',destino:'/contactos/aaaaaaaa-1111-4111-8111-dddddddddddd'})});});
  ...
  await expect(page.getByText('Contacto creado y clasificado desde el origen.')).toBeVisible();expect(posted.id_financiero_operativo).toBe('');expect(posted.nombre).toBe('María');expect(posted.tipo_contacto).toBe('cliente_hipoteca_particular');
 });
 test('Financiero crea el contacto para su propia identidad',async({page},testInfo)=>{
  ...
  await page.route('**/functions/v1/fenix-contactos-unified-test',async r=>{posted=JSON.parse(r.request().postData()||'{}');return r.fulfill({status:201,contentType:'application/json',body:JSON.stringify({ok:true,id:'bbbbbbbb-1111-4111-8111-dddddddddddd'})});});
 ...
 test('un duplicado no se vuelve a crear',async({page},testInfo)=>{
  ...
  await page.route('**/functions/v1/fenix-contactos-unified-test',r=>r.fulfill({status:409,contentType:'application/json',body:JSON.stringify({ok:false,error:'duplicate_contact',existing_id:'cccccccc-1111-4111-8111-dddddddddddd',destino:'/contactos/cccccccc-1111-4111-8111-dddddddddddd'})}));
 ...
 test('Visitador no recibe formulario de alta de cliente',async({page},testInfo)=>{
  if(!testInfo.project.name.includes('desktop'))test.skip();await boot(page,'Visitador');await page.goto('/contactos/nuevo');await expect(page.getByText('Tu perfil no puede crear contactos.')).toBeVisible();await expect(page.getByRole('button',{name:'Revisar antes de crear'})).toHaveCount(0);
 });
});
