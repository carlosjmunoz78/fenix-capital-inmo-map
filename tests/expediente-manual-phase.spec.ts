...
test('cambio manual de fase exige motivo, revisión y confirmación antes de escribir',async({page},testInfo)=>{
 ...
 await expect(page.getByTestId('manual-phase-panel')).toContainText('Pulsa cualquier fase');
 await page.getByRole('button',{name:'Cambiar manualmente a fase Análisis'}).click();
 await expect(page.getByTestId('manual-phase-panel')).toContainText('Entrada');
 await expect(page.getByTestId('manual-phase-panel')).toContainText('Estás saltando una o más fases');
 expect(writes).toBe(0);
 await page.getByPlaceholder('Explica el motivo operativo, excepción o evidencia que justifica el cambio…').fill('La documentación ya está completa y revisada.');
 await page.getByRole('button',{name:'Revisar cambio'}).click();
 expect(writes).toBe(0);
 await expect(page.getByText('¿Seguro que quieres cambiar')).toBeVisible();
 await expect(page.getByRole('button',{name:'Añadir documento justificativo'})).toBeVisible();
 await page.getByRole('button',{name:'Sí, cambiar fase'}).click();
 ...
 await expect(page.getByTestId('manual-phase-panel')).toContainText('Fase cambiada y motivo registrado.');
});
