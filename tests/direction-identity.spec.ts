...
async function boot(page:any,fullName?:string,personalItems:any[]=[]){
...
const greeting=/^(Hola Carlos, )?(Buenos días|Buenas tardes|Buenas noches|buenos días|buenas tardes|buenas noches)$/;
...
test.describe('Fénix PRE-PROD · identidad de Dirección',()=>{
 ...
 test('si la sesión no trae nombre usa solo la ficha canónica que coincide con actor_code',async({page},testInfo)=>{if(!testInfo.project.name.includes('desktop'))test.skip();await boot(page,undefined,[{actor_code:'CARLOS-ADMIN',name:'Carlos Muñoz',role:'Dirección'}]);const root=page.locator('.dir-shell');await expect(root.locator('.dir-user-copy strong')).toHaveText('Carlos Muñoz');await expect(root.locator('.dir-user-copy span')).toHaveText('Mi perfil');await expect(root.locator('.dir-priority-copy h1')).toHaveText(/^Hola Carlos, (buenos días|buenas tardes|buenas noches)$/);});
 test('sin identidad coincidente no inventa persona ni usa Dirección como nombre',async({page},testInfo)=>{if(!testInfo.project.name.includes('desktop'))test.skip();await boot(page);const root=page.locator('.dir-shell');await expect(root.locator('.dir-user-copy strong')).toHaveText('Usuario');await expect(root.locator('.dir-user-copy span')).toHaveText('Mi perfil');await expect(root.locator('.dir-priority-copy h1')).toHaveText(greeting);await expect(root.getByText(/Belén Muñoz|Hola Belén/)).toHaveCount(0);});
});
