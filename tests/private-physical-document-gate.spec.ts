...
function textOf(name:string){const file=join(dir,name);must(existsSync(file),`missing_fixture:${name}`);let text=execFileSync('pdftotext',['-layout',file,'-'],{encoding:'utf8',maxBuffer:24*1024*1024});if(!usableText(text))text=ocrPdf(file);must(usableText(text),`no_readable_text:${name}`);return text;}
