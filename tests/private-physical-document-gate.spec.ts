...
function identityShape(text:string){const upper=text.toLocaleUpperCase('es');const shape=[/NOMBRE(?:S)?|NAME|GIVEN NAMES/i.test(upper)?'nameLabel':'noNameLabel',/APELLIDOS?|SURNAME(?:S)?/i.test(upper)?'surnameLabel':'noSurnameLabel',/<<|<{2,}/.test(upper)?'mrzChevrons':'noMrzChevrons',/IDESP|ESP[A-Z0-9<]{5,}/.test(upper)?'mrzId':'noMrzId',/^\s*[A-ZÁÉÍÓÚÜÑ<]{4,}\s*$/m.test(upper)?'upperRun':'noUpperRun'];return shape.join('.')+`.lines${text.split(/\n+/).filter(Boolean).length}`;}
function payrollShape(text:string){const upper=text.toLocaleUpperCase('es');return[/N[ÓO]MINA|RECIBO DE SALARIOS|DEVENGOS/i.test(upper)?'payroll':'noPayroll',/TRABAJADOR|EMPLEAD|NOMBRE/i.test(upper)?'workerLabel':'noWorkerLabel',/EMPRESA|CIF|NIF/i.test(upper)?'companyLabel':'noCompanyLabel',/L[IÍ]QUIDO|NETO/i.test(upper)?'netLabel':'noNetLabel',/DEVENGADO|BRUTO/i.test(upper)?'grossLabel':'noGrossLabel'].join('.');}

physical('Notas Simples privadas reales rellenan lo soportado y no inventan',()=>{
 const names=['nota-simple-cristina.pdf','nota-simple-palomar.pdf','nota-simple-roncero.pdf'];let finca=0,desc=0,cadastral=0,charges=0;
 for(const name of names){const text=textOf(name);const {result,map}=card('Nota simple',text);const diag=flags(text);must(result.documentType==='Nota simple',`nota_family:${name}:flags=${diag}`);must(has(map,'Registro'),`nota_registro:${name}:flags=${diag}`);must(has(map,'Titular/es'),`nota_titulares:${name}:flags=${diag}`);if(has(map,'Número de finca'))finca++;if(has(map,'Descripción de finca'))desc++;if(has(map,'Referencia catastral'))cadastral++;if(has(map,'Cargas')||has(map,'Hipotecas')||has(map,'Embargos'))charges++;}
 must(finca>=2,'nota_finca_across_real_set');must(desc>=2,'nota_description_across_real_set');must(cadastral>=1,'nota_reference_cadastral_across_real_set');must(charges>=1,'nota_charges_across_real_set');
});

physical('recibo de préstamo privado real respeta ausencia de datos no soportados',()=>{const text=textOf('prestamo-personal.pdf');const {result,map}=card('Préstamo / deuda',text);const diag=flags(text);must(result.documentType==='Préstamo / deuda','loan_family');must(has(map,'Vencimiento')||has(map,'Referencia')||has(map,'Entidad')||has(map,'Cuota'),`loan_any_supported_field:flags=${diag}`);if(!/cap|saldo/.test(diag))must(!has(map,'Capital pendiente'),`loan_no_fabricated_balance:flags=${diag}`);if(!/cuota|rec/.test(diag))must(!has(map,'Cuota'),`loan_no_fabricated_payment:flags=${diag}`);});
...
physical('DNI privado real rellena identidad en la ficha visible',()=>{const text=textOf('dni-mari-francisco.pdf');const {result,map}=card('DNI / NIE',text);must(/DNI|NIE|Documento/i.test(result.documentType),'identity_family');must(has(map,'DNI / NIE'),'identity_number_visible');must(has(map,'Nombre y apellidos'),`identity_name_visible:${identityShape(text)}`);});

physical('Nómina privada real rellena campos laborales y económicos visibles',()=>{const text=textOf('nomina-real.pdf');const {result,map}=card('Nómina',text);const diag=payrollShape(text);must(result.documentType==='Nómina',`payroll_family:${diag}`);must(has(map,'Trabajador'),`payroll_worker_visible:${diag}`);must(has(map,'Empresa / CIF'),`payroll_company_visible:${diag}`);must(has(map,'Bruto'),`payroll_gross_visible:${diag}`);must(has(map,'Neto'),`payroll_net_visible:${diag}`);});
