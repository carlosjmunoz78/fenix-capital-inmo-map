...
function textOf(name:string){const file=join(dir,name);must(existsSync(file),`missing_fixture:${name}`);let text=execFileSync('pdftotext',['-layout',file,'-'],{encoding:'utf8',maxBuffer:24*1024*1024});if(text.trim().length<=120)text=ocrPdf(file);must(text.trim().length>120,`no_readable_text:${name}`);return text;}
...
function flags(text:string){const tests:[string,RegExp][]=[['desc',/DESCRIP/i],['urb',/URBANA/i],['rust',/R[ÚU]STICA/i],['viv',/VIVIENDA/i],['lind',/LINDER/i],['sup',/SUPERFICIE/i],['ent',/ENTIDAD/i],['acr',/ACREEDOR/i],['banco',/BANCO/i],['prest',/PR[ÉE]STAMO/i],['cap',/CAPITAL/i],['saldo',/SALDO/i],['pend',/PENDIENTE/i],['cuota',/CUOTA/i],['imp',/IMPORTE/i],['rec',/RECIBO/i],['amort',/AMORT/i],['deuda',/DEUDA/i],['cert',/CERTIFICADO/i],['cancel',/CANCEL/i]];return tests.filter(([,r])=>r.test(text)).map(([k])=>k).join('.')||'none';}
...
physical('tres Notas Simples privadas reales rellenan la ficha registral crítica',()=>{
 ...
 for(const name of names){const text=textOf(name);const {result,map}=card('Nota simple',text);const diag=flags(text);must(result.documentType==='Nota simple',`nota_family:${name}:flags=${diag}`);must(has(map,'Registro'),`nota_registro:${name}:flags=${diag}`);must(has(map,'Número de finca'),`nota_finca:${name}:flags=${diag}`);must(has(map,'Titular/es'),`nota_titulares:${name}:flags=${diag}`);must(has(map,'Descripción de finca'),`nota_descripcion:${name}:flags=${diag}`);if(has(map,'Referencia catastral'))cadastral++;if(has(map,'Cargas')||has(map,'Hipotecas')||has(map,'Embargos'))charges++;}
...
physical('recibo de préstamo privado real rellena deuda pendiente y cuota sin inventar',()=>{
 const text=textOf('prestamo-personal.pdf');const {result,map}=card('Préstamo / deuda',text);must(result.documentType==='Préstamo / deuda','loan_family');const bank=(text.match(/\b(?:BBVA|CAIXABANK|SANTANDER|SABADELL|UNICAJA|ABANCA|BANKINTER|CAJAMAR|CAJA RURAL)\b/i)||[])[0]||'none';const keys=Object.keys(result.fields).sort().join(',');const diag=flags(text);
 must(has(map,'Entidad'),`loan_entity:bank=${bank}:keys=${keys}:flags=${diag}`);must(has(map,'Capital pendiente'),`loan_balance:flags=${diag}`);must(has(map,'Cuota'),`loan_payment:flags=${diag}`);
});

physical('certificado privado real de deuda rellena acreedor y saldo pendiente',()=>{
 const text=textOf('certificado-deuda.pdf');const {result,map}=card('Certificado de deuda',text);must(/Préstamo \/ deuda|Documento|Factura \/ recibo/.test(result.documentType),'debt_certificate_family');const bank=(text.match(/\b(?:BBVA|CAIXABANK|SANTANDER|SABADELL|UNICAJA|ABANCA|BANKINTER|CAJAMAR|CAJA RURAL)\b/i)||[])[0]||'none';const keys=Object.keys(result.fields).sort().join(',');const diag=flags(text);
 must(has(map,'Acreedor'),`debt_creditor:bank=${bank}:keys=${keys}:flags=${diag}`);must(has(map,'Saldo pendiente'),`debt_balance:flags=${diag}`);
});
