...
test.describe('Fénix PRE-PROD · Visitador contacto B2B',()=>{
 test('abre y edita solo mediante gate B2B',async({page},testInfo)=>{
  ...
  await page.route(`**/functions/v1/fenix-b2b-actions-test/contactos/${contactId}**`,r=>{
   const isUpdate=r.request().method()==='POST'&&new URL(r.request().url()).pathname.endsWith(`/contactos/${contactId}/update`);
   if(isUpdate)return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true,status:200,id:contactId,destino:`/contactos-b2b/${contactId}`})});
