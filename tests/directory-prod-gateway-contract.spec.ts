...
test('notarias y registros usan gateway PROD directo sin suffix ni bloqueo legacy',()=>{
  ...
  expect(runtime).toContain('export const IS_PRODUCTION=true');
  expect(runtime).toContain('functions/v1/${baseFunctionName}${path}');
  expect(runtime).not.toContain('VITE_FUNCTION_SUFFIX');
