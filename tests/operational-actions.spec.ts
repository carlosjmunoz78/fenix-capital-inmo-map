...
test.describe('Fénix PRE-PROD · acciones Notion contextuales',()=>{
  test('Dirección guarda seguimiento canónico y refresca la ficha',async({page},testInfo)=>{
  ...
  test('403 de escritura se muestra como permiso correcto, sin falso éxito',async({page},testInfo)=>{
    if(!testInfo.project.name.includes('desktop'))test.skip();
    await prepare(page);
