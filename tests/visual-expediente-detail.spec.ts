...
const bankCandidates={ok:true,bancos:[{bank_code:'bank-1',nombre:'Banco Uno',active:true},{bank_code:'bank-2',nombre:'Banco Dos',active:true},{bank_code:'bank-3',nombre:'Banco Tres',active:true}]};
const bankSends={ok:true,envios:[{bank_code:'bank-1',estado:'Enviado'},{bank_code:'bank-1',estado:'Aprobado'},{bank_code:'bank-2',estado:'Enviado'},{bank_code:'bank-3',estado:'Enviado'}]};
const bankOffers={ok:true,ofertas:[{bank_code:'bank-1',estado:'Oferta recibida'},{bank_code:'bank-2',estado:'Oferta recibida'}]};
...
test.describe('Fénix PRE-PROD · ficha maestra de expediente',()=>{
 test('Ana usa datos vivos, contexto de Belén, ranking bancario, comunicación empática, recalcula sin recarga y prepara contacto idempotente',async({page},testInfo)=>{
  ...
  await page.route('**/functions/v1/fenix-app-gateway-test/**',async r=>{const u=r.request().url();if(u.endsWith('/session/context'))return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({actor_code:'DIR-TEST',role:'Direccion'})});if(u.endsWith('/navigation'))return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({items:[{label:'Inicio',route:'/inicio'},{label:'Expedientes',route:'/expedientes'}]})});if(u.endsWith(`/expedientes/${id}/bancos-candidatos`))return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify(bankCandidates)});if(u.endsWith(`/expedientes/${id}/envios-banco`))return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify(bankSends)});if(u.endsWith(`/expedientes/${id}/ofertas`))return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify(bankOffers)});return r.fulfill({status:404,body:'{}'});});
  ...
  await expect(page.getByTestId('expediente-bank-ranking-prod')).toBeVisible();
  await expect(page.getByText('Top 3 bancos para esta operación',{exact:true})).toBeVisible();
  await expect(page.getByText('Banco Uno',{exact:true})).toBeVisible();
  await expect(page.getByText(/OPCIÓN 1 · PUNTUACIÓN 47\/100/)).toBeVisible();
  await expect(page.getByText(/Solo se ordenan bancos con señales reales de envíos u ofertas/)).toBeVisible();
