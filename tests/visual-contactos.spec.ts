...
test.describe('Fénix PRE-PROD · contrato visual Contactos',()=>{
 test('Contactos usa patrón maestro con Ana, gráficos, filtros prefijados, sticky y ordenación',async({page},testInfo)=>{
  ...
  const kpis=page.locator('.contactos-kpis');
  await expect(kpis.getByText('CON SEGUIMIENTO',{exact:true})).toBeVisible();
  await expect(kpis.getByText('FORMALIZADOS',{exact:true})).toBeVisible();
  await expect(kpis.getByText('SIN PRÓXIMA ACCIÓN',{exact:true})).toBeVisible();
  ...
  await expect(table.locator('tbody tr').first().locator('td').first()).toContainText('CARMelo');
