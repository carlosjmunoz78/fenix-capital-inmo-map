...
test.describe('Fénix PRE-PROD · contrato visual Tasaciones',()=>{
 test('usa fuente canónica y no inventa valores ausentes',async({page},testInfo)=>{
  ...
  const addressHeader=table.locator('thead th').nth(0),dateHeader=table.locator('thead th').nth(3);
  const addressButton=addressHeader.locator('button'),dateButton=dateHeader.locator('button');
  expect(await addressHeader.evaluate(el=>getComputedStyle(el).position)).toBe('sticky');
  await expect(dateHeader).toHaveAttribute('aria-sort','descending');
  await expect(table.locator('tbody tr').first().locator('td').first()).toContainText('Calle Córdoba 8');
  await dateButton.click();
  await expect(dateHeader).toHaveAttribute('aria-sort','ascending');
  await expect(table.locator('tbody tr').first().locator('td').first()).toContainText('Av. de la Libertad 12');
  await addressButton.click();
  await expect(addressHeader).toHaveAttribute('aria-sort','ascending');
  await expect(table.locator('tbody tr').first().locator('td').first()).toContainText('Av. de la Libertad 12');
  await addressButton.click();
  await expect(addressHeader).toHaveAttribute('aria-sort','descending');
  await expect(table.locator('tbody tr').first().locator('td').first()).toContainText('Plaza Nueva 3');
