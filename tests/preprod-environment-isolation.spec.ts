...
test('workflow PRE-PROD permanece parado, manual, sin publicar ni escribir ramas',()=>{
  ...
  expect(text).toContain("VITE_FUNCTION_SUFFIX: '-test'");
  expect(text).toContain('Browser QA PRE-PROD');
  expect(text).toContain('Upload PRE-PROD dist');
  expect(text).not.toMatch(/git push/);
  expect(text).not.toContain('Publish exact validated APP snapshot');
});
