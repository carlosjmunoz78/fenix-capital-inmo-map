...
test('PRE-PROD aplica siempre sufijo -test a funciones Edge',()=>{
  ...
  expect(text).toContain("const runtimeEnv=(import.meta.env.VITE_FENIX_ENV||'').toLowerCase()");
...
test('cliente y almacenamiento de sesión mantienen aislamiento PRE-PROD/PROD',()=>{
  ...
  expect(text).toContain("const PROD_SUPABASE_URL='https://cluhljgonannaafpmblx.supabase.co'");
  expect(text).not.toContain('https://hnqlnvakzaywtafeiybt.supabase.co');
