...
const QA_ACCESS_TOKEN='eyJhbGciOiJub25lIiwidHlwIjoiSldUIn0.eyJhdWQiOiJhdXRoZW50aWNhdGVkIiwicm9sZSI6ImF1dGhlbnRpY2F0ZWQiLCJzdWIiOiJhYWFhYWFhYS1hYWFhLTRhYWEtOGFhYS1hYWFhYWFhYWFhYWEiLCJlbWFpbCI6InRlc3RAZmVuaXguZXMiLCJleHAiOjE5OTk5OTk5OTl9.';
function fakeSession(actorCode:string,email:string){return{access_token:QA_ACCESS_TOKEN,token_type:'bearer',expires_in:3600,expires_at:1999999999,refresh_token:'qa-inmo-access-not-real',user:{id:'aaaaaaaa-aaaa-4aaa-8aaa-aaaaaaaaaaaa',aud:'authenticated',role:'authenticated',email,app_metadata:{},user_metadata:{actor_code:actorCode},created_at:'2026-08-22T00:00:00.000Z'}};}
...
test.describe('Fénix PRE-PROD · acceso a nueva inmobiliaria por rol',()=>{
 ...
 test('Visitador crea solo por el endpoint B2B canónico, nunca por legacy',async({page},testInfo)=>{
  ...
  await expect(page.getByRole('button',{name:'Confirmar y crear'})).toBeVisible();
