...
test('shell operativo mantiene drawer autorizado, tema y calculadora utilizables en móvil',async({page},testInfo)=>{
 ...
 let drawer=page.getByRole('dialog',{name:'Navegación principal'});
 await expect(drawer).toBeVisible();
 let nav=drawer.locator('nav');
 ...
 await page.mouse.click((viewport?.width??393)-8,160);
 await expect(shell).toHaveAttribute('data-mobile-nav','closed');
 await menu.click();
 drawer=page.getByRole('dialog',{name:'Navegación principal'});
 nav=drawer.locator('nav');
