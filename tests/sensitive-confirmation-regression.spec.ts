...
test.describe('Fénix PRE-PROD · confirmación sensible transversal',()=>{
 ...
 test('edición de registro operativo no escribe hasta confirmar',async({page},testInfo)=>{
  ...
  await page.route(`**/functions/v1/fenix-notion-runtime-test/tareas/${id}`,r=>r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({item:{id,tarea:'Tarea segura',estado:'Pendiente',criticidad:'Normal'}})}));
  await page.route(`**/functions/v1/fenix-notion-actions-test/tareas/${id}/action`,r=>{posts++;return r.fulfill({status:200,contentType:'application/json',body:JSON.stringify({ok:true})});});
  await page.goto(`/tareas/${id}`);await page.getByLabel('Estado').selectOption('En curso');await page.getByRole('button',{name:'Revisar antes de guardar'}).click();expect(posts).toBe(0);await page.getByLabel('Criticidad').selectOption('Importante');await expect(page.getByRole('button',{name:'Confirmar y guardar'})).toHaveCount(0);expect(posts).toBe(0);await page.getByRole('button',{name:'Revisar antes de guardar'}).click();await page.getByRole('button',{name:'Confirmar y guardar'}).click();await expect.poll(()=>posts).toBe(1);
 });
