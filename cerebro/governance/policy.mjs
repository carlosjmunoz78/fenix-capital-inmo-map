...
function exceptionKey(ctx,eventId,action,reason,policy){return createHash('sha256').update(JSON.stringify([ctx.company_id,ctx.engine_id,ctx.environment,ctx.version,eventId,action,reason,policy??null])).digest('hex');}
