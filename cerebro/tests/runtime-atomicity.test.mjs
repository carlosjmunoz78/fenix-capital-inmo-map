...
test('EVT-001 does not reserve idempotency key when payload cloning fails', () => {
  const bus = new EventBus();
  assert.throws(() => bus.publish({ type: 'ATOMIC', payload: { bad: () => true }, context, idempotency_key: 'atomic-event' }), /clone|DataCloneError|could not be cloned/i);
...
test('JOB-001 does not reserve idempotency key when payload cloning fails', () => {
  const q = new JobQueue();
  assert.throws(() => q.enqueue({ name: 'atomic-job', payload: { bad: () => true }, context, idempotency_key: 'atomic-job-key' }), /clone|DataCloneError|could not be cloned/i);
...
test('JOB-001 completion remains RUNNING if result cloning fails', () => {
  const q = new JobQueue();
  q.enqueue({ name: 'complete-atomic', context, idempotency_key: 'complete-atomic-key' });
  const claimed = q.claimContext(context);
  assert.throws(() => q.completeContext(claimed.job_id, context, { bad: () => true }), /clone|DataCloneError|could not be cloned/i);
