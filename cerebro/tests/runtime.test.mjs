...
test('EVT-001 semantics are idempotent and tenant isolated', () => {
  const bus = new EventBus();
  const first = bus.publish({ type: 'LEAD_CREATED', payload: { lead_id: 1 }, context, idempotency_key: 'lead-1' });
  const duplicate = bus.publish({ type: 'LEAD_CREATED', payload: { lead_id: 1 }, context, idempotency_key: 'lead-1' });
  bus.publish({ type: 'LEAD_CREATED', payload: { lead_id: 2 }, context: { ...context, company_id: 'other-company' }, idempotency_key: 'lead-2' });
...
test('JOB-001 supports priority, retries, idempotency and tenant isolation', () => {
  const q = new JobQueue();
  q.enqueue({ name: 'low', context, priority: 100, idempotency_key: 'a' });
  q.enqueue({ name: 'high', context, priority: 1, max_attempts: 2, idempotency_key: 'b' });
  assert.equal(q.enqueue({ name: 'high', context, priority: 1, idempotency_key: 'b' }).duplicate, true);
  ...
  assert.equal(q.claim('other-company'), null);
});
