...
test('scheduler location expansion is explicit and one-to-one',()=>{
  const c=cfg.coverage_contract; assert.equal(c.scheduler_location_discovery_result_required,true); assert.equal(c.scheduler_expansion_parameter,'LOCATION'); assert.equal(c.scheduler_each_discovered_location_requires_exactly_one_jobs_result,true); assert.equal(c.scheduler_duplicate_or_missing_location_result_forbidden,true); assert.deepEqual(derivedIds('scheduler'),['GCP-READONLY-CATALOG:scheduler:0','GCP-READONLY-CATALOG:scheduler:1']);
});
