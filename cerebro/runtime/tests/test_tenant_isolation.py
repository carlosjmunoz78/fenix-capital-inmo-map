...
class TenantIsolationTests(unittest.TestCase):
    ...
    def test_unknown_company_is_denied(self):
        with self.assertRaises(TenantUnknownCompany):
            self.guard.authorize(
                authenticated_company_id="ghost-company",
                target_company_id="ghost-company",
                engine_id="CTX-001",
                version="0.1.0",
            )
    ...
    def test_mixed_company_records_are_rejected_not_filtered(self):
        ...
        with self.assertRaises(TenantIsolationError):
            self.guard.require_records(context, [
                {"company_id": "fenix-capital", "id": "1"},
                {"company_id": "acme-holdings", "id": "2"},
            ])
    ...
    def test_namespace_and_metrics_are_company_scoped(self):
        ...
        self.assertEqual(
            self.guard.namespace(context, "dataset", "keywords"),
            "company/fenix-capital/dataset/keywords",
        )
