...
function assertResolvedParameters(v,projectId,label){
  if(!v||typeof v!=='object'||Array.isArray(v)) throw new TypeError(`${label} must be an object`);
  if(v.PROJECT_ID!==projectId) throw new Error(`${label}.PROJECT_ID must equal record.project_id`);
  for(const [k,val] of Object.entries(v)){assertCanonicalString(k,`${label} key`); assertCanonicalString(val,`${label}.${k}`);}
}
...
function validateCommandResults(record,label){
  ...
  for(const [i,res] of record.command_results.entries()){
    ...
    assertExactKeys(res,expectedFields,rl); assertCanonicalString(res.command_run_ref,`${rl}.command_run_ref`); if(seenRuns.has(res.command_run_ref)) throw new Error(`${label} duplicate command_run_ref`); seenRuns.add(res.command_run_ref);
    assertTemplateRef(res.command_template_id_or_ref,record.domain,`${rl}.command_template_id_or_ref`); if(!RESULT_STATUS.has(res.result_status)) throw new RangeError(`${rl}.result_status invalid`); assertTimestamp(res.captured_at,`${rl}.captured_at`); assertPrincipalRef(res.principal_ref,`${rl}.principal_ref`); assertEvidenceRef(res.evidence_ref,`${rl}.evidence_ref`); assertResolvedParameters(res.resolved_parameters,record.project_id,`${rl}.resolved_parameters`);
    if(isSchedulerDiscovery){if(!Array.isArray(res.discovered_values)) throw new TypeError(`${rl}.discovered_values must be an array`); for(const v of res.discovered_values) assertCanonicalString(v,`${rl}.discovered_values[]`);}
    const arr=byTemplate.get(res.command_template_id_or_ref)||[]; arr.push(res); byTemplate.set(res.command_template_id_or_ref,arr);
  }
  if(record.domain!=='scheduler'){
    for(const template of expected){const arr=byTemplate.get(template); if(!arr||arr.length!==1) throw new Error(`${label} requires exactly one result for ${template}`);}
    return byTemplate;
  }
  ...
  const jobs=byTemplate.get(expected[1])||[];
...
export function validateGcpInventoryEnvelope(text){
  const input=parseEnvelopeJson(text); const seenCaptureIds=new Set(); const coveragePairs=new Set(); const coverageByPair=new Map();
