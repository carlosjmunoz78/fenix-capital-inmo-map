...
Deno.serve(async req=>{
 ...
 if(action==='update_user'){
  const target=String(body?.actor_code||''),displayName=String(body?.display_name||''),role=String(body?.role||''),active=Boolean(body?.active);
  const{data,error}=await svc.rpc('fenix_prod_user_admin_update_server',{p_actor_code:ctx.actor_code,p_target_actor_code:target,p_display_name:displayName,p_role:role,p_active:active});
  if(error)return out(req,{ok:false,error:'user_update_failed'},500);return out(req,data,Number(data?.status)||200);
 }
 if(action==='set_goal'){
  const scopeType=String(body?.scope_type||''),scopeCode=String(body?.scope_code||''),metric=String(body?.metric_code||''),target=Number(body?.target_value);
  const{data,error}=await svc.rpc('fenix_prod_profile_goal_set_server',{p_actor_code:ctx.actor_code,p_scope_type:scopeType,p_scope_code:scopeCode,p_metric_code:metric,p_target_value:target,p_company_id:'FENIX'});
  if(error)return out(req,{ok:false,error:'goal_update_failed'},500);return out(req,data,Number(data?.status)||200);
 }
