...
Deno.serve(async(req:Request)=>{try{
 ...
 if(isTest(req))return out(req,{ok:true,status:200,test:true,updated:fields});
 if(ot==='contacto'){
   ...
   const intelligence={document_type:s(b.document_type,80)||'Documento',summary:s(b.summary,1900),document_page_id:s(b.document_page_id,100)||null,confidence:Number.isFinite(Number(b.confidence))?Number(b.confidence):null,updated_by:ctx.actor_code,updated_at:new Date().toISOString()};
   ...
   return out(req,{ok:true,status:200,updated:u.data,conflicts_resolved:conflicts.length});
 }
 if(!['Direccion','Financiero'].includes(String(ctx.role)))return out(req,{ok:false,error:'forbidden'},403);
 ...
 if(r.error)return out(req,{ok:false,error:'expediente_update_failed'},500);return out(req,r.data,Number(r.data?.status??200));
