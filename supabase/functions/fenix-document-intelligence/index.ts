...
Deno.serve(async(req:Request)=>{try{
 ...
 const policy=await svc.rpc('fenix_prod_runtime_policy_server',{p_policy_key:'document_auto_ingest_min_confidence'});if(policy.error||!policy.data?.ok)return out(req,{ok:false,error:'confidence_policy_unavailable',human_required:true,human_reason:'POLICY_CONFLICT'},503);const minConfidence=Number(policy.data?.value?.min_confidence);if(!Number.isFinite(minConfidence)||minConfidence<0||minConfidence>1)return out(req,{ok:false,error:'confidence_policy_invalid',human_required:true,human_reason:'POLICY_CONFLICT'},503);
 const payload={version:6,environment:'PROD',origin_type:ot,origin_code:oc,declared_document_type:declaredType,declared_person:declaredPerson,detected_document_type:detected,summary,confidence,fields:all,actor_code:ctx.actor_code,processed_at:new Date().toISOString(),policy:{key:'document_auto_ingest_min_confidence',version:Number(policy.data.version),min_confidence:minConfidence}};
 if(confidence===null||confidence<minConfidence){if(docPage){const st=await store(docPage,payload);if(!st.ok)return out(req,{ok:false,error:st.error},500)}return out(req,{ok:false,status:409,error:'low_confidence',review_required:true,human_required:true,human_reason:'LOW_CONFIDENCE',confidence,min_confidence:minConfidence,policy_version:Number(policy.data.version),extraction:{document_type:detected,person:declaredPerson,confidence,summary,fields:all}},409)}
