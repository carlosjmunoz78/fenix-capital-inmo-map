...
Deno.serve(async(req:Request)=>{
 ...
 const actorCode=String(ctx.actor_code||''),url=new URL(req.url),path=url.pathname.replace(/^\/functions\/v1\/fenix-profile-api/,'')||'/';
 if(req.method==='GET'&&path==='/goals'){
   const{data,error}=await svc.rpc('fenix_prod_profile_goals_get_server',{p_actor_code:actorCode,p_company_id:'FENIX'});
   if(error)return out(req,{ok:false,error:'goals_read_failed'},500);return out(req,data,statusOf(data));
 }
 if(req.method==='GET'){
   const{data,error}=await svc.rpc('fenix_prod_profile_get_full_server',{p_actor_code:actorCode});
   if(error)return out(req,{ok:false,error:'profile_read_failed'},500);return out(req,data,statusOf(data));
 }
 if(path!=='/')return out(req,{ok:false,error:'not_found'},404);
 let body:any={};try{body=await req.json()}catch{return out(req,{ok:false,error:'invalid_json'},400)}
 const{data,error}=await svc.rpc('fenix_prod_profile_update_full_server',{p_actor_code:actorCode,p_profile:body});
 if(error)return out(req,{ok:false,error:'profile_update_failed'},500);return out(req,data,statusOf(data));
});
