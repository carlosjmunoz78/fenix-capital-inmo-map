...
Deno.serve(async(req:Request)=>{try{
 ...
 const actor=String(ctxRes.data.actor_code),role=normalize(ctxRes.data.role||'');if(!['direccion','financiero'].includes(role))return out(req,{ok:false,error:'forbidden'},403);
