...
const TERMINAL=new Set(['firmado','cerrado','cierre','finalizado','baja','perdido','pausado']);
...
type Candidate={upload_id:string;filename:string;document_id:string;bytes:number;session:any;run:any};
...
function normalize(v:unknown){return clean(v,100).normalize('NFD').replace(/[\u0300-\u036f]/g,'').toLowerCase()}
...
function repairChanges(fields:any){const x:Record<string,unknown>={};if(!fields||typeof fields!=='object')return x;if(fields.documento_identidad)x.dni_nie=fields.documento_identidad;if(fields.nombre)x.nombre=fields.nombre;if(fields.apellidos)x.apellidos=fields.apellidos;if(fields.fecha_nacimiento)x.fecha_nacimiento=fields.fecha_nacimiento;if(fields.empresa)x.empresa_organismo=fields.empresa;if(fields.situacion_laboral_actual)x.situacion_laboral=fields.situacion_laboral_actual;if(fields.antiguedad_laboral)x.antiguedad_laboral=fields.antiguedad_laboral;if(fields.ingresos_netos_mensuales!==undefined)x.sueldo_neto_mensual=fields.ingresos_netos_mensuales;if(fields.otros_ingresos_mensuales!==undefined)x.otros_ingresos_mensuales=fields.otros_ingresos_mensuales;if(fields.cuotas_deuda_mensuales!==undefined)x.deudas_mensuales=fields.cuotas_deuda_mensuales;if(fields.ahorros!==undefined)x.ahorro_disponible=fields.ahorros;return x}
...
Deno.serve(async(req:Request)=>{try{
 ...
 const{data:ud,error:ue}=await user.auth.getUser(auth.slice(7));if(ue||!ud.user)return out(req,{ok:false,error:'unauthorized'},401);
 const{data:ctxRes,error:ce}=await svc.rpc('fenix_prod_actor_context_by_auth_server',{p_auth_user_id:ud.user.id});if(ce||!ctxRes?.ok||!ctxRes?.actor_code)return out(req,{ok:false,error:'identity_not_linked'},403);
 const actor=String(ctxRes.actor_code),role=normalize(ctxRes.role||'');if(!['direccion','financiero'].includes(role))return out(req,{ok:false,error:'forbidden'},403);
 ...
 const exp=await svc.schema('fenix_prod').from('expedientes').select('id,stage').eq('expediente_code',expCode).maybeSingle();if(exp.error||!exp.data?.id)return out(req,{ok:false,error:'expediente_not_found'},404);if(TERMINAL.has(normalize(exp.data.stage)))return out(req,{ok:true,status:200,processed:0,succeeded:0,failed:0,remaining:0,skipped:[{error:'terminal_stage'}],items:[]});
 ...
 const uploadIds=(sessions.data??[]).map((s:any)=>String(s.id));const runs=uploadIds.length?await svc.schema('fenix_prod').from('document_intelligence_runs').select('upload_id,status,extraction,error,updated_at').in('upload_id',uploadIds).order('updated_at',{ascending:false}):{data:[],error:null};if(runs.error)return out(req,{ok:false,error:'runs_list_failed'},500);
 ...
 const byPath=new Map((sessions.data??[]).map((s:any)=>[String(s.storage_path),s]));const candidates:Candidate[]=[];for(const d of docs.data??[]){const v=current.get(String(d.id));if(!v||v.bytes>MAX_BYTES)continue;const s:any=byPath.get(v.storage_path);if(!s)continue;const run=latest.get(String(s.id));if(run?.status==='applied')continue;candidates.push({upload_id:String(s.id),filename:String(s.filename||d.title||''),document_id:String(d.id),bytes:v.bytes,session:s,run})}
 const repairs:Candidate[]=candidates.filter(c=>String(c.session?.origin_type)==='comprador'&&c.run?.status==='needs_review'&&String(c.run?.error)==='participant_identity_mismatch'&&people.some(p=>String(p.id||p.contact_code)===String(c.session?.origin_code))&&!!personByExistingDni(c.run,people));
 const repairResults:any[]=[];for(const c of repairs.slice(0,MAX_ITEMS)){
   const target=personByExistingDni(c.run,people)!;const contactCode=String(target.id||target.contact_code||'');const fields=c.run?.extraction?.fields||{};const changes=repairChanges(fields);
   const u=await svc.rpc('fenix_prod_exp_person_update_server',{p_actor_code:actor,p_client_code:contactCode,p_exp_code:expCode,p_changes:changes});
   if(u.error||u.data?.ok===false){repairResults.push({file:c.filename,ok:false,error:u.data?.error||'identity_repair_contact_apply_failed',person:contactCode});continue}
   const extraction={...(c.run?.extraction||{}),contact_code:contactCode,identity_repair:{source:'existing_extraction',reason:'participant_identity_mismatch',evidence:'document_dni_unique_in_expediente',previous_origin_type:String(c.session?.origin_type||''),previous_origin_code:String(c.session?.origin_code||''),corrected_contact_code:contactCode}};
   const done=await svc.rpc('fenix_prod_document_extract_run_update_server',{p_upload_id:c.upload_id,p_status:'applied',p_extraction:extraction,p_error:null});
   if(done.error||done.data?.ok===false){repairResults.push({file:c.filename,ok:false,error:'identity_repair_run_update_failed',person:contactCode});continue}
   repairResults.push({file:c.filename,ok:true,status:200,error:null,type:'identity_repair',person:String(target.comprador||`${target.nombre??''} ${target.apellidos??''}`.trim())});
 }
 if(repairResults.length){const succeeded=repairResults.filter(x=>x.ok).length;return out(req,{ok:true,status:200,processed:repairResults.length,succeeded,failed:repairResults.length-succeeded,remaining:Math.max(0,candidates.length-repairResults.length),skipped:[],items:repairResults});}
 const routes:Route[]=[],skipped:any[]=[];for(const c of candidates){if(String(c.session?.origin_type)!=='expediente'||String(c.session?.origin_code)!==expCode){skipped.push({file:c.filename,error:'non_expediente_session'});continue}const rule=ruleFor(c.filename,people);if(!rule){skipped.push({file:c.filename,error:'unrecognized_filename'});continue}const person=rule.personRequired?routedPerson(c.filename,people,c.run):null;if(rule.personRequired&&!person){skipped.push({file:c.filename,error:'ambiguous_person'});continue}const contactCode=String(person?.id||person?.contact_code||'');const personName=person?String(person.comprador||[person.nombre,person.apellidos].filter(Boolean).join(' ')):'Expediente';routes.push({upload_id:c.upload_id,filename:c.filename,document_id:c.document_id,type:rule.type,family:rule.family,person:personName,contactCode})}
