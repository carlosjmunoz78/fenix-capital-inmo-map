...
declare me text; new_name text:=left(btrim(coalesce(p_profile->>'display_name','')),120); new_username text:=nullif(left(btrim(coalesce(p_profile->>'username','')),80),''); bd date; begin
 if auth.uid() is null then return jsonb_build_object('ok',false,'status',401,'error','unauthorized'); end if;
 ...
 insert into fenix_prod.audit_events(actor_code,event_type,target_type,target_code,metadata) values(me,'profile.updated','actor_profile',me,jsonb_build_object('fields',jsonb_build_array('display_name','username','birth_date','contact_email','phone','job_title','zone','bio','linkedin','instagram','website','avatar_url')));
