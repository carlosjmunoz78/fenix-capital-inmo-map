...
begin
  ...
  if allowed then
    -- Intelligence is read independently from upload linkage. A historic/dangling
    -- run must never supply the upload id used for a new extraction attempt.
    select r.extraction,r.status,r.updated_at
      into v_extraction,v_intel_status,v_intel_updated_at
      from fenix_prod.document_intelligence_runs r
     where r.document_id=d.id
     order by r.updated_at desc nulls last
     limit 1;

    -- Always resolve an upload from a CURRENT document version joined to an
    -- actually existing completed upload session. This prevents stale run ids
    -- from producing upload_not_found in the extractor.
    select us.id into v_upload_id
     ...
     order by dv.created_at desc nulls last, us.created_at desc nulls last
     limit 1;
  end if;
