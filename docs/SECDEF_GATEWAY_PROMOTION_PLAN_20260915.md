...
- Repository main: `680e3ae541d8fc36847e143a05ed0514298c04b6`.
...
- Latest PROD runtime smoke on main SHA `680e3ae...` is GREEN for primary Gateway health/fail-closed behavior and critical anonymous-denial checks.
...
3. deploy canonical main Gateway source as a new primary version without touching legacy grants;
4. verify `/health`, environment and configuration;
5. run authenticated and denied HTTP checks for all 15 contracted capabilities;
6. run OLD-vs-NEW parity for read routes;
7. run controlled mutation E2E only with isolated/reversible test records;
8. on any failure, restore the captured primary v18 source and leave legacy grants untouched;
9. after successful rollback proof, recover/promote NEW and re-run health/parity;
10. only after this entire sequence is GREEN may legacy retirement be considered in a separate lot.
